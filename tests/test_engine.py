...
from aivision.models.aivi import classify, detect, obb, pose, segment, semantic
from aivision.nn.tasks import load_checkpoint
from aivision.utils import ASSETS, DEFAULT_CFG, IS_RASPBERRYPI, WEIGHTS_DIR
...
def test_export(monkeypatch, tmp_path):
    ...
    f = exporter(model=AIVI("aivi26n.yaml").model)
    AIVI(f)(SOURCE)  # exported model inference
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (obb.OBBTrainer, obb.OBBValidator, obb.OBBPredictor, "dota8.yaml", "aivi26n-obb.yaml", None),
        (pose.PoseTrainer, pose.PoseValidator, pose.PosePredictor, "coco8-pose.yaml", "aivi26n-pose.yaml", None),
        (
...
def test_task(trainer_cls, validator_cls, predictor_cls, data, model, weights):
    """Test AIVI training, validation, and prediction for various tasks."""
    overrides = {
        "data": data,
        "model": model,
        "imgsz": 32,
    ...
    if model == "aivi26n.yaml":  # only for detection
...
def test_load_checkpoint_state_dict_rejected(ckpt, tmp_path):
    ...
    with pytest.raises(TypeError, match="supported AIVision checkpoint format"):
...
def test_nan_recovery():
    ...
    overrides = {"data": "coco8.yaml", "model": "aivi26n.yaml", "imgsz": 32, "epochs": 3}
...
def test_checkpoint_fp16_overflow():
    ...
    overrides = {"data": "coco8.yaml", "model": "aivi26n.yaml", "imgsz": 32, "epochs": 2}
    trainer = detect.DetectionTrainer(overrides=overrides)
...
def test_train_reuses_loaded_checkpoint_model(monkeypatch, kwargs, uses_weights):
    ...
    monkeypatch.setattr("aivision.engine.model.checks.check_pip_update_available", lambda: None)
    monkeypatch.setattr(model, "_smart_load", lambda key: FakeTrainer)
    monkeypatch.setattr(
        "aivision.engine.model.load_checkpoint",
...
def test_setup_model_respects_pretrained_arg_for_pt_models(monkeypatch, pretrained, uses_weights):
    ...
    monkeypatch.setattr(
        "aivision.engine.trainer.load_checkpoint", lambda path: (checkpoint_model, {"checkpoint": True})
    )
