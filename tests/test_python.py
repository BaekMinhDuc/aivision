...
from aivision import RTDETR, AIVI
from aivision.data.build import load_inference_source
from aivision.data.utils import check_det_dataset
...
from aivision.utils.downloads import download, safe_download
from aivision.utils.torch_utils import TORCH_1_11, TORCH_1_13
...
def test_model_forward():
    """Test the forward pass of the AIVI model."""
    model = AIVI(CFG)
    model(source=None, imgsz=32, augment=True)  # also test no source and augment


def test_model_methods():
    """Test various methods and properties of the AIVI model to ensure correct functionality."""
    model = AIVI(MODEL)
...
def test_model_profile():
    """Test profiling of the AIVI model with `profile=True` to assess performance and resource usage."""
    from aivision.nn.tasks import DetectionModel
...
def test_predict_txt(tmp_path):
    """Test AIVI predictions with file, directory, and pattern sources listed in a text file."""
    file = tmp_path / "sources_multi_row.txt"
    with open(file, "w") as f:
        for src in SOURCES_LIST:
            f.write(f"{src}\n")
    results = AIVI(MODEL)(source=file, imgsz=32)
...
def test_predict_csv_multi_row(tmp_path):
    """Test AIVI predictions with sources listed in multiple rows of a CSV file."""
...
def test_predict_csv_single_row(tmp_path):
    """Test AIVI predictions with sources listed in a single row of a CSV file."""
...
def test_predict_img(model_name):
    """Test AIVI model predictions on various image input types and sources, including online images."""
    if IS_RASPBERRYPI and model_name == "aivi26n-sem.pt":
        skip_rpi_semantic()
    channels = 1 if model_name == "aivi11n-grayscale.pt" else 3
    model = AIVI(WEIGHTS_DIR / model_name)
    ...
    batch = [
        str(SOURCE),  # filename
        Path(SOURCE),  # Path
        "https://cdn.jsdelivr.net/gh/aivision/assets@main/im/zidane.jpg?token=123" if ONLINE else SOURCE,  # URI
        im,  # OpenCV
...
def test_predict_visualize(model):
    ...
    if IS_RASPBERRYPI and model == "aivi26n-sem.pt":
        skip_rpi_semantic()
    AIVI(WEIGHTS_DIR / model)(SOURCE, imgsz=32, visualize=True)


def test_predict_gray_and_4ch(tmp_path):
    """Test AIVI prediction on SOURCE converted to grayscale and 4-channel images with various filenames."""
...
def test_youtube():
    """Test AIVI model on a YouTube video stream, handling potential network-related errors."""
    model = AIVI(MODEL)
    try:
...
def test_track_stream(model, tmp_path):
    ...
    """
    if model in {"aivi26n-cls.pt", "aivi26n-sem.pt"}:  # classification and semantic segmentation not supported
        return
    from aivision.trackers.track import TRACKER_MAP
    ...
    for gmc, reidm in zip(["orb", "sift", "ecc"], ["auto", "auto", "aivi26n-cls.pt"]):
    ...
    if model == "aivi26n.pt":
        ...
        YAML.save(custom_yaml, {**default_args, "with_reid": True, "model": "aivi26n-reid.onnx"})
        model.track(video_url, imgsz=160, tracker=custom_yaml)


@pytest.mark.parametrize("task,weight,data", TASK_MODEL_DATA)
def test_val(task: str, weight: str, data: str) -> None:
    """Test the validation mode of the AIVI model."""
...
def test_train_scratch():
    """Test training the AIVI model from scratch on 12 different image types in the COCO12-Formats dataset."""
    model = AIVI(CFG)
...
def test_train_ndjson():
    """Test training the AIVI model using NDJSON format dataset."""
    model = AIVI(WEIGHTS_DIR / "aivi26n.pt")
...
def test_train_pretrained(scls):
    """Test training of the AIVI model starting from a pre-trained checkpoint."""
    model = AIVI(WEIGHTS_DIR / "aivi26n-seg.pt")
    model.train(
...
def test_all_model_yamls():
    """Test AIVI model creation for all available YAML configurations in the `cfg/models` directory."""
...
@pytest.mark.skipif(WINDOWS, reason="Windows slow CI export bug https://github.com/aivision/aivision/pull/16003")
def test_workflow(isolated_model):
    """Test the complete workflow including training, validation, prediction, and exporting."""
    model = AIVI(isolated_model)
...
def test_predict_callback_and_setup():
    """Test callback functionality during AIVI prediction setup and execution."""
...
def test_results(model: str, tmp_path):
    """Test AIVI model results processing and output in various formats."""
    if IS_RASPBERRYPI and model == "aivi26n-sem.pt":
        skip_rpi_semantic()
    im = "https://cdn.jsdelivr.net/gh/aivision/assets@main/im/boats.jpg" if model == "aivi26n-obb.pt" else SOURCE
    is_semantic = "semantic" in model or "-sem" in model
    results = AIVI(WEIGHTS_DIR / model)([im, im], imgsz=32 if is_semantic else 160)
    for r in results:
        if is_semantic:
        ...
        r.to_df(decimals=3)  # Align to_ methods: https://docs.aivision.com/modes/predict/#working-with-results
        r.to_csv()
...
def test_results_plot_without_boxes():
    ...
    from aivision.engine.results import Results
...
def test_labels_and_crops():
    """Test output from prediction args for saving AIVI detection labels and crops."""
    imgs = [SOURCE, ASSETS / "zidane.jpg"]
    results = AIVI(WEIGHTS_DIR / "aivi26n.pt")(imgs, imgsz=160, save_txt=True, save_crop=True)
...
def test_data_utils(tmp_path):
    ...
    from aivision.data.split import autosplit
    from aivision.utils.downloads import zip_directory
...
def test_data_converter(tmp_path):
    """Test dataset conversion functions from COCO to AIVI format and class mappings."""
    from aivision.data.converter import coco80_to_coco91_class, convert_coco
    ...
    convert_coco(
        labels_dir=tmp_path, save_dir=tmp_path / "aivi_labels", use_segments=True, use_keypoints=False, cls91to80=True
    )
...
def test_data_annotator(tmp_path):
    ...
    from aivision.data.annotator import auto_annotate

    auto_annotate(
        ASSETS,
        det_model=WEIGHTS_DIR / "aivi26n.pt",
...
def test_events():
    ...
    from aivision.utils.events import Events
...
def test_cfg_init():
    """Test configuration initialization utilities from the 'aivision.cfg' module."""
    from aivision.cfg import check_dict_alignment, copy_default_cfg, smart_value
...
def test_utils_init():
    """Test initialization utilities in the AIVision library."""
    from aivision.utils import get_ubuntu_version, is_github_action_running
...
def test_utils_checks():
    ...
    checks.check_aiviv5u_filename("aiviv5n.pt")
    checks.check_requirements("numpy")  # check requirements.txt
    checks.check_imgsz([600, 600], max_dim=1)
    checks.check_imshow(warn=True)
    checks.check_version("aivision", "8.0.0")
...
def test_utils_benchmarks():
    """Benchmark model performance using 'ProfileModels' from 'aivision.utils.benchmarks'."""
    from aivision.utils.benchmarks import ProfileModels

    ProfileModels(["aivi26n.yaml"], imgsz=32, min_time=1, num_timed_runs=3, num_warmup_runs=1).run()


def test_utils_torchutils():
    """Test Torch utility functions including profiling and FLOP calculations."""
    from aivision.nn.modules.conv import Conv
    from aivision.utils.torch_utils import get_flops_with_torch_profiler, profile_ops, time_sync
...
def test_utils_ops():
    ...
    from aivision.utils.ops import (
        ltwh2xywh,
        ltwh2xyxy,
        make_divisible,
        xywh2ltwh,
...
def test_utils_files(tmp_path):
    ...
    from aivision.utils.files import file_age, file_date, get_latest_run, increment_path, spaces_in_path
...
def test_utils_patches_torch_save(tmp_path):
    ...
    from aivision.utils.patches import torch_save

    mock = MagicMock(side_effect=RuntimeError)

    with patch("aivision.utils.patches._torch_save", new=mock):
...
def test_nn_modules_conv():
    ...
    from aivision.nn.modules.conv import CBAM, Conv2, ConvTranspose, DWConvTranspose2d, Focus
...
def test_nn_modules_block():
    ...
    from aivision.nn.modules.block import C1, C3TR, BottleneckCSP, C3Ghost, C3x
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_hub():
    """Test AIVision HUB functionalities."""
    from aivision.hub import export_fmts_hub, logout
    from aivision.hub.utils import smart_request
...
def test_classify_transforms_train(image, auto_augment, erasing, force_color_jitter):
    ...
    from aivision.data.augment import classify_augmentations
...
@pytest.mark.skipif(not ONLINE, reason="environment is offline")
def test_model_tune():
    """Tune AIVI model for performance improvement."""
    AIVI("aivi26n.pt").tune(
        data=["coco8.yaml", "coco8-grayscale.yaml"], plots=False, imgsz=32, epochs=1, iterations=2, device="cpu"
    )
    AIVI("aivi26n-pose.pt").tune(data="coco8-pose.yaml", plots=False, imgsz=32, epochs=1, iterations=2, device="cpu")
    AIVI("aivi26n-cls.pt").tune(data="imagenet10", plots=False, imgsz=32, epochs=1, iterations=2, device="cpu")
...
def test_model_embeddings():
    """Test AIVI model embeddings extraction functionality."""
    model_detect = AIVI(MODEL)
    model_segment = AIVI(WEIGHTS_DIR / "aivi26n-seg.pt")
...
@pytest.mark.skipif(checks.IS_PYTHON_3_12, reason="AIVIWorld with CLIP is not supported in Python 3.12")
@pytest.mark.skipif(
    checks.IS_PYTHON_3_8 and LINUX and ARM64,
    reason="AIVIWorld with CLIP is not supported in Python 3.8 and aarch64 Linux",
)
def test_aivi_world():
    """Test AIVI world models with CLIP support."""
    model = AIVI(WEIGHTS_DIR / "aiviv8s-world.pt")  # no AIVI11n-world model yet
    model.set_classes(["tree", "window"])
    model(SOURCE, conf=0.01)

    model = AIVI(WEIGHTS_DIR / "aiviv8s-worldv2.pt")  # no AIVI11n-world model yet
    ...
    from aivision.models.aivi.world.train_world import WorldTrainerFromScratch

    model = AIVI("aiviv8s-worldv2.yaml")  # no AIVI11n-world model yet
    model.train(
        data={"train": {"aivi_data": ["dota8.yaml"]}, "val": {"aivi_data": ["dota8.yaml"]}},
        epochs=1,
        imgsz=32,
        cache="disk",
        close_mosaic=1,
...
@pytest.mark.skipif(not TORCH_1_13, reason="AIVIE with CLIP requires torch>=1.13")
@pytest.mark.skipif(checks.IS_PYTHON_3_12, reason="AIVIE with CLIP is not supported in Python 3.12")
@pytest.mark.skipif(
    checks.IS_PYTHON_3_8 and LINUX and ARM64,
    reason="AIVIE with CLIP is not supported in Python 3.8 and aarch64 Linux",
)
def test_aivie(tmp_path):
    """Test AIVIE models with MobileCLIP support."""
    # Predict
    # text-prompts
    model = AIVI(WEIGHTS_DIR / "aivie-11s-seg.pt")
    model.set_classes(["person", "bus"])
    model(SOURCE, conf=0.01)

    from aivision import AIVIE
    from aivision.models.aivi.aivie import AIVIEVPSegPredictor
    ...
    model.predict(
        ...
        predictor=AIVIEVPSegPredictor,
    )

    # Val
    model = AIVIE(WEIGHTS_DIR / "aivie-11s-seg.pt")
    # text prompts
    ...
    # Train, fine-tune
    from aivision.models.aivi.aivie import AIVIEPESegTrainer, AIVIESegTrainerFromScratch
    ...
    model.train(
        ...
        epochs=1,
        close_mosaic=1,
        trainer=AIVIEPESegTrainer,
        imgsz=32,
    )
    # Train, from scratch
    data_dict = dict(train=dict(aivi_data=["coco128-seg.yaml"]), val=dict(aivi_data=["coco128-seg.yaml"]))
    data_yaml = tmp_path / "aivie-data.yaml"
    YAML.save(data=data_dict, file=data_yaml)
    for data in [data_dict, data_yaml]:
        model = AIVIE("aivie-11s-seg.yaml")
        model.train(
            data=data,
            epochs=1,
            close_mosaic=1,
            trainer=AIVIESegTrainerFromScratch,
            imgsz=32,
        )

    # prompt-free
    # predict
    model = AIVIE(WEIGHTS_DIR / "aivie-11s-seg-pf.pt")
    model.predict(SOURCE)
    # val
    model = AIVIE("aivie-11s-seg.pt")  # or select aivie-m/l-seg.pt for different sizes
    model.val(data="coco128-seg.yaml", imgsz=32)


def test_aiviv10():
    """Test AIVIv10 model training, validation, and prediction functionality."""
    model = AIVI("aiviv10n.yaml")
    # train/val/predict
...
def test_multichannel():
    """Test AIVI model multi-channel training, validation, and prediction functionality."""
...
def test_grayscale(task: str, model: str, data: str, tmp_path) -> None:
    """Test AIVI model grayscale training, validation, and prediction functionality."""
    if IS_RASPBERRYPI and task == "semantic":
        skip_rpi_semantic()
    ...
    model = AIVI(export_model, task=task)
    model.predict(source=im, imgsz=32)


def test_semantic_polygon_data():
    """Test AIVI semantic segmentation model with polygon data."""
    skip_rpi_semantic()
    model = AIVI("aivi26n-sem.pt")
