...
def solution_assets():
    ...
    from aivision.utils import ASSETS_URL, WEIGHTS_DIR
...
def pytest_sessionstart(session):
    ...
    """
    from aivision.utils.torch_utils import init_seeds
...
def pytest_sessionfinish(session, exitstatus):
    ...
    from aivision.utils import WEIGHTS_DIR

    # Remove files
    models = [path for x in {"*.onnx", "*.torchscript"} for path in WEIGHTS_DIR.rglob(x)]
    for file in ["decelera_portrait_min.mov", "bus.jpg", "aivi26n.onnx", "aivi26n.torchscript", *models]:
