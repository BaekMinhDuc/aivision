...
from aivision.utils import ASSETS, IS_JETSON, WEIGHTS_DIR
from aivision.utils.autodevice import GPUInfo
from aivision.utils.checks import check_amp, check_tensorrt
from aivision.utils.torch_utils import TORCH_1_13
...
if CUDA_IS_AVAILABLE:
    if IS_JETSON:
        ...
        autodevice_fraction = __import__("os").environ.get("AIVI_AUTODEVICE_FRACTION_FREE", 0.3)
...
def test_amp():
    ...
    model = AIVI("aivi26n.pt").model.to(f"cuda:{DEVICES[0]}")
...
def test_export_onnx_matrix(task, dynamic, int8, half, batch, simplify, nms):
    """Test AIVI exports to ONNX format with various configurations and parameters."""
    ...
    AIVI(file)([SOURCE] * batch, imgsz=64 if dynamic else 32, device=DEVICES[0])  # exported model inference
...
def test_export_engine_matrix(task, dynamic, int8, half, batch):
    """Test AIVI model export to TensorRT format for various configurations and run inference."""
    check_tensorrt()
...
def test_train():
    ...
    results = AIVI(MODEL).train(data="coco8-grayscale.yaml", imgsz=64, epochs=1, device=DEVICES[0], batch=-1)
    results = AIVI(MODEL).train(data="coco8.yaml", imgsz=64, epochs=1, device=device, batch=15, compile=True)
    results = AIVI(MODEL).train(data="coco128.yaml", imgsz=64, epochs=1, device=device, batch=15, val=False)
...
@pytest.mark.skipif(not DEVICES, reason="No CUDA devices available")
def test_autobatch():
    """Check optimal batch size for AIVI model training using autobatch utility."""
    from aivision.utils.autobatch import check_train_batch_size

    check_train_batch_size(AIVI(MODEL).model.to(f"cuda:{DEVICES[0]}"), imgsz=64, amp=True)


@pytest.mark.slow
@pytest.mark.skipif(not DEVICES, reason="No CUDA devices available")
def test_utils_benchmarks(isolated_model):
    """Profile AIVI models for performance benchmarks."""
    from aivision.utils.benchmarks import ProfileModels

    # Pre-export a dynamic engine model to use dynamic inference
    AIVI(isolated_model).export(format="engine", imgsz=32, dynamic=True, batch=1, device=DEVICES[0])
    ProfileModels(
        [isolated_model],
        imgsz=32,
        half=False,
...
def test_predict_sam():
    ...
    from aivision.models.sam import Predictor as SAMPredictor
