...
from aivision import AIVI, download
from aivision.utils import ASSETS_URL, DATASETS_DIR, SETTINGS
...
def test_tensorboard():
    ...
    AIVI("aivi26n-cls.yaml").train(data="imagenet10", imgsz=32, epochs=3, plots=False, device="cpu")
    SETTINGS["tensorboard"] = False


@pytest.mark.skipif(not check_requirements("ray", install=False), reason="ray[tune] not installed")
def test_model_ray_tune():
    """Tune AIVI model using Ray for hyperparameter optimization."""
    AIVI("aivi26n-cls.yaml").tune(
...
@pytest.mark.skipif(True, reason="Test failing in scheduled CI https://github.com/aivision/aivision/pull/8868")
@pytest.mark.skipif(not check_requirements("mlflow", install=False), reason="mlflow not installed")
def test_mlflow_keep_run_active():
    ...
    AIVI("aivi26n-cls.yaml").train(data="imagenet10", imgsz=32, epochs=1, plots=False, device="cpu")
...
def test_triton(tmp_path, isolated_model):
    """Test NVIDIA Triton Server functionalities with AIVI model."""
    ...
    f = AIVI(isolated_model).export(format="onnx", dynamic=True)
    ...
    AIVI(f"http://localhost:8000/{model_name}", "detect")(SOURCE)  # exported model inference
...
def test_faster_coco_eval():
    """Validate AIVI model predictions on COCO dataset using faster-coco-eval."""
    from aivision.models.aivi.detect import DetectionValidator
    from aivision.models.aivi.pose import PoseValidator
    from aivision.models.aivi.segment import SegmentationValidator

    args = {"model": "aivi26n.pt", "data": "coco8.yaml", "save_json": True, "imgsz": 64}
    ...
    args = {"model": "aivi26n-seg.pt", "data": "coco8-seg.yaml", "save_json": True, "imgsz": 64}
    ...
    args = {"model": "aivi26n-pose.pt", "data": "coco8-pose.yaml", "save_json": True, "imgsz": 64}
