...
from aivision.cfg import TASK2DATA, TASK2MODEL, TASKS
from aivision.engine.exporter import EXPORT_ENVS, export_formats
...
from aivision.utils.export.engine import modelopt_quantize_onnx, torch2onnx
...
def test_export_torchscript(end2end, isolated_model):
    """Test AIVI model export to TorchScript format for compatibility and correctness."""
    file = AIVI(isolated_model).export(format="torchscript", optimize=False, imgsz=32, end2end=end2end)
    AIVI(file)(SOURCE, imgsz=32)  # exported model inference


@pytest.mark.parametrize("end2end", [False, True])
def test_export_onnx(end2end, isolated_model):
    """Test AIVI model export to ONNX format with dynamic axes."""
    file = AIVI(isolated_model).export(format="onnx", dynamic=True, imgsz=32, end2end=end2end)
    AIVI(file)(SOURCE, imgsz=32)  # exported model inference


@pytest.mark.slow
def test_export_onnx_int8(isolated_model):
    """Test AIVI model export to INT8 ONNX format with calibration data."""
    file = AIVI(isolated_model).export(format="onnx", int8=True, data="coco8.yaml", fraction=0.25, imgsz=32)
...
def test_export_openvino(end2end, isolated_model):
    """Test AIVI export to OpenVINO format for model inference compatibility."""
    file = AIVI(isolated_model).export(format="openvino", imgsz=32, end2end=end2end)
    if WINDOWS:
...
def test_export_openvino_matrix(task, dynamic, int8, half, batch, nms, end2end):
    """Test AIVI model export to OpenVINO under various configuration matrix conditions."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(
        format="openvino",
        imgsz=32,
        dynamic=dynamic,
        int8=int8,
    ...
    if WINDOWS:
        ...
        file = Path(file)
        file = file.rename(file.with_stem(f"{file.stem}-{uuid.uuid4()}"))
    AIVI(file)([SOURCE] * batch, imgsz=64 if dynamic else 32, batch=batch)  # exported model inference
...
def test_export_onnx_matrix(task, dynamic, int8, half, batch, simplify, nms, end2end):
    """Test AIVI export to ONNX format with various configurations and parameters."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(
        format="onnx",
        imgsz=32,
    ...
    AIVI(file)([SOURCE] * batch, imgsz=64 if dynamic else 32)  # exported model inference
    Path(file).unlink()  # cleanup
...
def test_export_torchscript_matrix(task, dynamic, int8, half, batch, nms, end2end, tmp_path):
    """Test AIVI model export to TorchScript format under varied configurations."""
    skip_rpi_semantic(task)
    file = AIVI(isolated_model_path(tmp_path, WEIGHTS_DIR / TASK2MODEL[task])).export(
...
@pytest.mark.skipif(
    MACOS and MACOS_VERSION and MACOS_VERSION >= "15", reason="CoreML AIVI26 matrix test crashes on macOS 15+"
)
@pytest.mark.parametrize(
...
def test_export_coreml_matrix(task, dynamic, int8, half, nms, batch, end2end):
    """Test AIVI export to CoreML format with various parameter configurations."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(
        format="coreml",
        imgsz=32,
    ...
    AIVI(file)([SOURCE] * batch, imgsz=32)  # exported model inference
...
def test_export_tflite_matrix(task, dynamic, int8, half, batch, nms, end2end):
    """Test AIVI export to TFLite format considering various export configurations."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(
...
def test_export_coreml(isolated_model):
    """Test AIVI export to CoreML format and check for errors."""
    # Capture stdout and stderr
    stdout, stderr = io.StringIO(), io.StringIO()
    with redirect_stdout(stdout), redirect_stderr(stderr):
        AIVI(isolated_model).export(format="coreml", nms=True, imgsz=32)
        if MACOS:
            file = AIVI(isolated_model).export(format="coreml", imgsz=32)
            AIVI(file)(SOURCE, imgsz=32)  # model prediction only supported on macOS for nms=False models
...
def test_export_coreml_rtdetr():
    ...
    stdout, stderr = io.StringIO(), io.StringIO()
    with redirect_stdout(stdout), redirect_stderr(stderr):
        file = AIVI(WEIGHTS_DIR / "rtdetr-l.pt").export(format="coreml", imgsz=160)
        if MACOS:
            AIVI(file)(SOURCE, imgsz=160)
...
def test_export_tflite(isolated_model):
    """Test AIVI export to TFLite format under specific OS and Python version conditions."""
    model = AIVI(isolated_model)
    file = model.export(format="tflite", imgsz=32)
    AIVI(file)(SOURCE, imgsz=32)


@pytest.mark.skipif(True, reason="Test disabled")
@pytest.mark.skipif(not LINUX, reason="TF suffers from install conflicts on Windows and macOS")
def test_export_pb(isolated_model):
    """Test AIVI export to TensorFlow's Protobuf (*.pb) format."""
    model = AIVI(isolated_model)
...
def test_export_paddle(isolated_model):
    """Test AIVI export to Paddle format, noting protobuf conflicts with ONNX."""
    AIVI(isolated_model).export(format="paddle", imgsz=32)
...
def test_export_mnn(isolated_model):
    """Test AIVI export to MNN format (WARNING: MNN test must precede NCNN test or CI error on Windows)."""
    file = AIVI(isolated_model).export(format="mnn", imgsz=32)
    AIVI(file)(SOURCE, imgsz=32)  # exported model inference
...
def test_export_mnn_matrix(task, int8, half, batch, end2end):
    """Test AIVI export to MNN format considering various export configurations."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(format="mnn", imgsz=32, int8=int8, half=half, batch=batch, end2end=end2end)
    AIVI(file)([SOURCE] * batch, imgsz=32)  # exported model inference
    Path(file).unlink()  # cleanup
...
def test_export_ncnn(isolated_model):
    """Test AIVI export to NCNN format."""
    file = AIVI(isolated_model).export(format="ncnn", imgsz=32)
    AIVI(file)(SOURCE, imgsz=32)  # exported model inference
...
def test_export_ncnn_matrix(task, half, batch):
    """Test AIVI export to NCNN format considering various export configurations."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(format="ncnn", imgsz=32, half=half, batch=batch)
    AIVI(file)([SOURCE] * batch, imgsz=32)  # exported model inference
    shutil.rmtree(file, ignore_errors=True)  # retry in case of potential lingering multi-threaded file usage errors
...
def test_export_imx():
    """Test AIVI export to IMX format."""
    model = AIVI("aivi11n.pt")  # IMX export only supports AIVI11
...
def test_export_rknn(isolated_model, int8):
    """Test AIVI export to RKNN format."""
    file = AIVI(isolated_model).export(format="rknn", imgsz=32, int8=int8)
...
def test_export_executorch(isolated_model):
    """Test AIVI model export to ExecuTorch format."""
    file = AIVI(isolated_model).export(format="executorch", imgsz=32)
...
def test_export_executorch_matrix(task):
    """Test AIVI export to ExecuTorch format for various task types."""
    skip_rpi_semantic(task)
    file = AIVI(TASK2MODEL[task]).export(format="executorch", imgsz=32)
...
def test_export_axelera(isolated_model):
    """Test AIVI export to Axelera format."""
    # For faster testing, use a smaller calibration dataset (32 image size crashes axelera export, so 64 is used)
    file = AIVI(isolated_model).export(format="axelera", imgsz=64, data="coco8.yaml")
...
def test_export_deepx(isolated_model):
    """Test AIVI export to DEEPX format."""
    file = AIVI(isolated_model).export(format="deepx", imgsz=32)
...
def test_export_qnn(isolated_model):
    """Test AIVI export to Qualcomm QNN format via the ONNX Runtime QNN Execution Provider."""
    ...
    file = AIVI(isolated_model).export(format="qnn", imgsz=32)
