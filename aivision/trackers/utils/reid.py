...
* `.pt` AIVI checkpoints — loaded via `AIVI()`; embeddings are pulled from the second-to-last
...
REID_ASSETS = frozenset(f"aivi26{k}-reid.onnx" for k in "nsmlx")


class ReID:
    """ReID encoder. Routes `.pt` to the AIVI predictor path; everything else to `AutoBackend`."""

    def __init__(self, model: str, imgsz: int = 224, device: str | torch.device | None = None, fp16: bool = False):
        """Initialize encoder for re-identification.

        Args:
            model (str): Path to a ReID model. `.pt` runs through the AIVI predictor (embed-layer extraction); other
