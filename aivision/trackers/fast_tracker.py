...
class FastSTrack(STrack):
    ...
    Examples:
        >>> from aivision.trackers.utils.kalman_filter import KalmanFilterXYAH
...
class FASTTracker(BYTETracker):
    ...
    Examples:
        Plug FastTracker into a AIVI model via the bundled config:
        >>> from aivision import AIVI
        >>> model = AIVI("aivi26s.pt")
        >>> model.track("video.mp4", tracker="fasttrack.yaml")

        Drive FastTracker directly with your own detections:
        >>> from aivision.trackers import FASTTracker
        >>> from aivision.utils import YAML, IterableSimpleNamespace
        >>> cfg = IterableSimpleNamespace(**YAML.load("aivision/cfg/trackers/fasttrack.yaml"))
