...
TASK2MODEL = {
    "detect": "aivi26n.pt",
    "segment": "aivi26n-seg.pt",
    "classify": "aivi26n-cls.pt",
    "pose": "aivi26n-pose.pt",
    "obb": "aivi26n-obb.pt",
    "semantic": "aivi26n-sem.pt",
}
TASK2METRIC = {
...
SOLUTIONS_HELP_MSG = f"""
    Arguments received: {["aivi", *ARGV[1:]]!s}. AIVision 'aivi solutions' usage overview:

        aivi solutions SOLUTION ARGS

        Where SOLUTION (optional) is one of {list(SOLUTION_MAP.keys())[:-1]}
              ARGS (optional) are any number of custom 'arg=value' pairs like 'show_in=True' that override defaults
                  at https://docs.aivision.com/usage/cfg

    1. Call object counting solution
        aivi solutions count source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    2. Call heatmap solution
        aivi solutions heatmap colormap=cv2.COLORMAP_PARULA model=aivi26n.pt

    3. Call queue management solution
        aivi solutions queue region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]" model=aivi26n.pt

    4. Call workout monitoring solution for push-ups
        aivi solutions workout model=aivi26n-pose.pt kpts=[6, 8, 10]

    5. Generate analytical graphs
        aivi solutions analytics analytics_type="pie"

    6. Track objects within specific zones
        aivi solutions trackzone source="path/to/video.mp4" region="[(150, 150), (1130, 150), (1130, 570), (150, 570)]"

    7. Count objects inside specific regions
        aivi solutions region source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    8. Run security alarm monitoring (email alerts require Python API)
        aivi solutions security source="path/to/video.mp4"

    9. Monitor parking occupancy (create JSON annotations first via Python ParkingPtsSelection)
        aivi solutions parking source="path/to/video.mp4" json_file="bounding_boxes.json"

    10. Streamlit real-time webcam inference GUI
        aivi streamlit-predict
    """
CLI_HELP_MSG = f"""
    Arguments received: {["aivi", *ARGV[1:]]!s}. AIVision 'aivi' commands use the following syntax:
        ...
        Where   TASK (optional) is one of {list(TASKS)}
                ...
                ARGS (optional) are any number of custom 'arg=value' pairs like 'imgsz=320' that override defaults.
                    See all ARGS at https://docs.aivision.com/usage/cfg or with 'aivi cfg'

    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        aivi train data=coco8.yaml model=aivi26n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        aivi predict model=aivi26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Validate a pretrained detection model at batch-size 1 and image size 640:
        aivi val model=aivi26n.pt data=coco8.yaml batch=1 imgsz=640

    4. Export a AIVI26n classification model to ONNX format at image size 224 by 128 (no TASK required)
        aivi export model=aivi26n-cls.pt format=onnx imgsz=224,128

    5. AIVision solutions usage
        aivi solutions count or any of {list(SOLUTION_MAP.keys())[1:-1]} source="path/to/video.mp4"
    ...
    Solutions: https://docs.aivision.com/solutions/
    Community: https://community.aivision.com
    GitHub: https://github.com/aivision/aivision
    """
...
def get_cfg(
    ...
    Examples:
        >>> from aivision.cfg import get_cfg
...
def check_cfg(cfg: dict, hard: bool = True) -> None:
    """Check configuration argument types and values for the AIVision library.
...
def get_save_dir(args: SimpleNamespace, name: str | None = None) -> Path:
    ...
    if getattr(args, "save_dir", None):
        ...
        from aivision.utils.files import increment_path
...
def check_dict_alignment(
    ...
    if mismatched := [k for k in custom_keys if k not in base_keys and k not in allowed_custom_keys]:
        ...
        for x in mismatched:
            ...
            string += f"'{colorstr('red', 'bold', x)}' is not a valid AIVI argument. {match_str}\n"
...
def handle_aivi_hub(args: list[str]) -> None:
    """Handle AIVision HUB command-line interface (CLI) commands for authentication.

    This function processes AIVision HUB CLI commands such as login and logout. It should be called when executing a
    ...
    Examples:
        $ aivi login YOUR_API_KEY

    Notes:
        - The function imports the 'hub' module from aivision to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from aivision import hub

    if args[0] == "login":
        key = args[1] if len(args) > 1 else ""
        # Log in to AIVision HUB using the provided API key
        hub.login(key)
    elif args[0] == "logout":
        # Log out from AIVision HUB
        hub.logout()


def handle_aivi_settings(args: list[str]) -> None:
    """Handle AIVI settings command-line interface (CLI) commands.

    This function processes AIVI settings CLI commands such as reset and updating individual settings. It should be
    called when executing a script with arguments related to AIVI settings management.

    Args:
        args (list[str]): A list of command line arguments for AIVI settings management.

    Examples:
        >>> handle_aivi_settings(["reset"])  # Reset AIVI settings
        >>> handle_aivi_settings(["runs_dir=path/to/dir"])  # Update a specific setting
    ...
    Notes:
        ...
        - For more information on handling AIVI settings, visit:
          https://docs.aivision.com/quickstart/#aivision-settings
    """
    url = "https://docs.aivision.com/quickstart/#aivision-settings"  # help URL
    try:
        if any(args):
            if args[0] == "reset":
        ...
        LOGGER.info(f"💡 Learn more about AIVision Settings at {url}")
    except Exception as e:
        LOGGER.warning(f"settings error: '{e}'. Please see {url} for help.")


def handle_aivi_solutions(args: list[str]) -> None:
    """Process AIVI solutions arguments and run the specified computer vision solutions pipeline.

    Args:
        args (list[str]): Command-line arguments for configuring and running the AIVision AIVI solutions.

    Examples:
        Run people counting solution with default settings:
        >>> handle_aivi_solutions(["count"])

        Run analytics with custom configuration:
        >>> handle_aivi_solutions(["analytics", "conf=0.25", "source=path/to/video.mp4"])

        Run inference with custom configuration, requires Streamlit version 1.29.0 or higher.
        >>> handle_aivi_solutions(["inference", "model=aivi26n.pt"])
    ...
    Notes:
        ...
        - The Streamlit app file is located in the AIVision package directory.
    """
    from aivision.solutions.config import SolutionConfig
    ...
    # Get solution name
    if not args:
        LOGGER.warning("No solution name provided. i.e `aivi solutions count`. Defaulting to 'count'.")
        args = ["count"]
    if args[0] == "help":
    ...
    if solution_name == "inference":
        ...
        LOGGER.info("💡 Loading AIVision live inference app...")
        subprocess.run(
            [  # Run subprocess with Streamlit custom argument
                ...
                "--server.headless",
                "true",
                overrides.pop("model", "aivi26n.pt"),
            ]
        )
    else:
        import cv2  # Only needed for cap and vw functionality

        from aivision import solutions
...
def parse_key_value_pair(pair: str = "key=value") -> tuple:
    ...
    Examples:
        >>> key, value = parse_key_value_pair("model=aivi26n.pt")
        >>> print(f"Key: {key}, Value: {value}")
        Key: model, Value: aivi26n.pt
...
def entrypoint(debug: str = "") -> None:
    """AIVision entrypoint function for parsing and executing command-line arguments.

    This function serves as the main entry point for the AIVision CLI, parsing command-line arguments and executing
    ...
    Examples:
        ...
        >>> entrypoint("train data=coco8.yaml model=aivi26n.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation model at image size 320:
        >>> entrypoint("predict model=aivi26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained detection model at batch-size 1 and image size 640:
        >>> entrypoint("val model=aivi26n.pt data=coco8.yaml batch=1 imgsz=640")

    Notes:
        - If no arguments are passed, the function will display the usage help message.
        - For a list of all available commands and their arguments, see the provided help messages and the
          AIVision documentation at https://docs.aivision.com.
    """
    ...
    special = {
        ...
        "settings": lambda: handle_aivi_settings(args[1:]),
        "cfg": lambda: YAML.print(DEFAULT_CFG_PATH),
        "hub": lambda: handle_aivi_hub(args[1:]),
        "login": lambda: handle_aivi_hub(args),
        "logout": lambda: handle_aivi_hub(args),
        "copy-cfg": copy_default_cfg,
        "solutions": lambda: handle_aivi_solutions(args[1:]),
    ...
    for a in merge_equals_args(args):  # merge spaces around '=' sign
        ...
        if "=" in a:
            ...
            overrides[a] = True  # auto-True for default bool args, i.e. 'aivi show' sets show=True
        elif a in DEFAULT_CFG_DICT:
            raise SyntaxError(
                f"'{colorstr('red', 'bold', a)}' is a valid AIVI argument but is missing an '=' sign "
    ...
    if "rtdetr" in stem:  # guess architecture
        ...
        from aivision import FastSAM

        model = FastSAM(model)
    elif "sam_" in stem or "sam2_" in stem or "sam2.1_" in stem:
        from aivision import SAM

        model = SAM(model)
    else:
        from aivision import AIVI

        model = AIVI(model, task=task)
        if "aivie" in stem or "world" in stem:
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        overrides["source"] = (
            "https://aivision.com/images/boats.jpg" if task == "obb" else DEFAULT_CFG.source or ASSETS
        )
    ...
    # Show help
    LOGGER.info(f"💡 Learn more at https://docs.aivision.com/modes/{mode}")
...
def copy_default_cfg() -> None:
    ...
    Examples:
        ...
        # Example AIVI command with this new custom cfg:
        #   aivi cfg='/path/to/current/directory/default_copy.yaml' imgsz=320 batch=8

    Notes:
        - The new configuration file is created in the current working directory.
        - After copying, the function prints a message with the new file's location and an example
          AIVI command demonstrating how to use the new configuration file.
    ...
    LOGGER.info(
        ...
        f"Example AIVI command with this new custom cfg:\n    aivi cfg='{new_file}' imgsz=320 batch=8"
    )


if __name__ == "__main__":
    # Example: entrypoint(debug='aivi predict model=aivi26n.pt')
    entrypoint(debug="")
