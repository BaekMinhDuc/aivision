...
from aivision.solutions.solutions import BaseSolution, SolutionResults
...
class InstanceSegmentation(BaseSolution):
    ...
    Attributes:
        model (AIVI): The segmentation model instance used for inference.
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        Args:
            **kwargs (Any): Keyword arguments passed to the BaseSolution parent class including:
                - model (str): Model name or path, defaults to "aivi26n-seg.pt".
        """
        kwargs["model"] = kwargs.get("model", "aivi26n-seg.pt")
    ...
    def process(self, im0) -> SolutionResults:
        ...
        if self.masks is None:
            self.LOGGER.warning("No masks detected! Ensure you're using a supported AIVision segmentation model.")
            plot_im = im0
        else:
