...
from aivision.utils import ASSETS_URL, LOGGER, ops
from aivision.utils.checks import check_imshow, check_requirements
from aivision.utils.plotting import Annotator


class BaseSolution:
    """A base class for managing AIVision Solutions.

    This class provides core functionality for various AIVision Solutions, including model loading, object tracking,
    ...
    Attributes:
        ...
        tracks: AIVI tracking results from the latest inference.
        ...
        model (AIVI): Loaded AIVI model instance.
    ...
    Examples:
        >>> solution = BaseSolution(model="aivi26n.pt", region=[(0, 0), (100, 0), (100, 100), (0, 100)])
    ...
    def __init__(self, is_cli: bool = False, **kwargs: Any) -> None:
        """Initialize the BaseSolution class with configuration settings and AIVI model.
        ...
        self.LOGGER.info(f"AIVision Solutions: ✅ {self.CFG}")
        self.region = self.CFG["region"]  # Store region data for other classes usage
        self.line_width = self.CFG["line_width"]

        # Load Model and store additional information (classes, show_conf, show_label)
        if self.CFG["model"] is None:
            self.CFG["model"] = "aivi26n.pt"
        self.model = AIVI(self.CFG["model"])
        ...
        }  # verbose must be passed to track method; setting it False in AIVI still logs the track information.
        ...
        if is_cli and self.CFG["source"] is None:
            ...
            safe_download(f"{ASSETS_URL}/{d_s}")  # download source from aivision assets
    ...
    def display_output(self, plot_im: np.ndarray) -> None:
        ...
        if self.CFG.get("show") and self.env_check:
            cv2.imshow("AIVision Solutions", plot_im)
...
class SolutionAnnotator(Annotator):
    ...
    trails, and visual annotations for AIVision Solutions. It offers comprehensive visualization capabilities for
...
class SolutionResults:
    """A class to encapsulate the results of AIVision Solutions.
