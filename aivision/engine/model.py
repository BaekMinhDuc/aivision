...
from aivision.cfg import TASK2DATA, get_cfg, get_save_dir
from aivision.engine.results import Results
from aivision.nn.tasks import guess_model_task, load_checkpoint, yaml_model_load
...
class Model(torch.nn.Module):
    """A base class for implementing AIVI models, unifying APIs across different model types.

    This class provides a common interface for various operations related to AIVI models, such as training, validation,
    prediction, exporting, and benchmarking. It handles different types of models, including those loaded from local
    files, AIVision HUB, or Triton Server.
    ...
    Attributes:
        ...
        metrics (aivision.utils.metrics.DetMetrics): The latest training/validation metrics.
        session (HUBTrainingSession): The AIVision HUB session, if applicable.
    ...
    def __init__(
        self,
        model: str | Path | Model = "aivi26n.pt",
        task: str | None = None,
        verbose: bool = False,
    ) -> None:
        """Initialize a new instance of the AIVI model class.

        This constructor sets up the model based on the provided model path or name. It handles various types of model
        sources, including local files, AIVision HUB models, and Triton Server models. The method initializes several
        important attributes of the model and prepares it for operations like training, prediction, or export.

        Args:
            model (str | Path | Model): Path or name of the model to load or create. Can be a local file path, a model
                name from AIVision HUB, a Triton Server model, or an already initialized Model instance.
        ...
        # Check if AIVision HUB model from https://hub.aivision.com
        if self.is_hub_model(model):
            from aivision.hub import HUBTrainingSession
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Returns:
            (list[aivision.engine.results.Results]): A list of prediction results, each encapsulated in a Results
                object.

        Examples:
            >>> model = AIVI("aivi26n.pt")
            >>> results = model("https://aivision.com/images/bus.jpg")
            >>> for r in results:
    ...
    def is_triton_model(model: str) -> bool:
        ...
        Examples:
            >>> Model.is_triton_model("http://localhost:8000/v2/models/aivi11n")
            True
            >>> Model.is_triton_model("aivi26n.pt")
            False
        """
    ...
    def is_hub_model(model: str) -> bool:
        """Check if the provided model is an AIVision HUB model.

        This static method determines whether the given model string represents a valid AIVision HUB model
        identifier.
        ...
        Returns:
            (bool): True if the model is a valid AIVision HUB model, False otherwise.

        Examples:
            >>> Model.is_hub_model("https://hub.aivision.com/models/MODEL")
            True
            >>> Model.is_hub_model("aivi26n.pt")
            False
        """
        from aivision.hub import HUB_WEB_ROOT
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        Examples:
            >>> model = Model()
            >>> model._new("aivi26n.yaml", task="detect", verbose=True)
        """
    ...
    def _load(self, weights: str, task=None) -> None:
        ...
        Examples:
            >>> model = Model()
            >>> model._load("aivi26n.pt")
            >>> model._load("path/to/weights.pth", task="detect")
        """
        if weights.lower().startswith(checks.REMOTE_FILE_PREFIXES):
            weights = checks.check_file(weights, download_dir=SETTINGS["weights_dir"])  # download and return local file
        weights = checks.check_model_file_from_stem(weights)  # add suffix, i.e. aivi26n -> aivi26n.pt
    ...
    def _check_is_pytorch_model(self) -> None:
        ...
        Examples:
            >>> model = Model("aivi26n.pt")
            >>> model._check_is_pytorch_model()  # No error raised
            >>> model = Model("aivi26n.onnx")
        ...
        if not (pt_module or pt_str):
            raise TypeError(
                ...
                f"i.e. 'aivi predict model=aivi26n.onnx'.\nTo run CUDA or MPS inference please pass the device "
    ...
    def load(self, weights: str | Path = "aivi26n.pt") -> Model:
    ...
    def save(self, filename: str | Path = "saved_model.pt") -> None:
        ...
        date, AIVision version, license information, and a link to the documentation.
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        Examples:
            >>> model = AIVI("aivi26n.pt")
            >>> image = "https://aivision.com/images/bus.jpg"
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        """Perform predictions on the given image source using the AIVI model.
        ...
        """
        if source is None:
            source = "https://aivision.com/images/boats.jpg" if self.task == "obb" else ASSETS
            LOGGER.warning(f"'source' is missing. Using 'source={source}'.")

        is_cli = (ARGV[0].endswith("aivi") or ARGV[0].endswith("aivision")) and any(
    ...
    def track(
        ...
        **kwargs: Any,
        ...
        Returns:
            (list[aivision.engine.results.Results]): A list of tracking results, each a Results object.
        ...
        if not hasattr(self.predictor, "trackers"):
            from aivision.trackers import register_tracker
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        Args:
            validator (aivision.engine.validator.BaseValidator, optional): An instance of a custom validator class
                for validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (aivision.utils.metrics.DetMetrics): Validation metrics obtained from the validation process. The
    ...
    def benchmark(self, data=None, format="", verbose=False, **kwargs: Any):
        ...
        uses the 'benchmark' function from the aivision.utils.benchmarks module. The benchmarking is configured using
        ...
        """
        self._check_is_pytorch_model()
        from aivision.utils.benchmarks import benchmark
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        checkpoint, integrating with AIVision HUB, and updating model and configuration after training.

        When using AIVision HUB, if the session has a loaded model, the method prioritizes HUB training arguments and
        ...
        Returns:
            (aivision.utils.metrics.DetMetrics | None): Training metrics if available and training is successful;
        ...
        """
        self._check_is_pytorch_model()
        if hasattr(self.session, "model") and self.session.model.id:  # AIVision HUB session with loaded model
            if any(kwargs):
    ...
    def tune(
        ...
        **kwargs: Any,
        ...
        is enabled, it leverages the 'run_ray_tune' function from the aivision.utils.tuner module. Otherwise, it uses
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from aivision.utils.tuner import run_ray_tune
    ...
    def names(self) -> dict[int, str]:
        ...
        using the 'check_class_names' function from the aivision.nn.autobackend module. If the predictor is not
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the AIVision callback system.
        ...
        Notes:
            - This method affects both custom callbacks added by the user and default callbacks
              provided by the AIVision framework.
