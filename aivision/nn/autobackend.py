...
from aivision.utils.checks import check_suffix
from aivision.utils.downloads import is_url
...
def default_class_names(data: str | Path | None = None) -> dict[int, str]:
    ...
    if data:
        try:
            ...
            from aivision.utils.checks import check_yaml
...
class AutoBackend(nn.Module):
    """Handle dynamic backend selection for running inference using AIVision AIVI models.
    ...
    Attributes:
        ...
        stride (int): The model stride, typically 32 for AIVI models.
    ...
    Examples:
        >>> model = AutoBackend(model="aivi26n.pt", device="cuda")
        >>> results = model(img)
    """
    ...
    def __init__(
        self,
        model: str | torch.nn.Module = "aivi26n.pt",
        ...
        if format == "tfjs":
            raise NotImplementedError("AIVision TF.js inference is not currently supported.")
        if format not in self._BACKEND_MAP:
            ...
            raise TypeError(
                ...
                f"AIVision supports: {export_formats()['Format']}\n"
                f"See https://docs.aivision.com/modes/predict for help."
            )
        if format == "pt":
    ...
    def warmup(self, imgsz: tuple[int, int, int, int] = (1, 3, 640, 640)) -> None:
        ...
        """
        from aivision.utils.nms import non_max_suppression
