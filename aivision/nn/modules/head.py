...
from aivision.utils import NOT_MACOS14
from aivision.utils.tal import dist2bbox, dist2rbox, make_anchors
from aivision.utils.torch_utils import TORCH_1_11, fuse_conv_and_bn, smart_inference_mode
...
class Detect(nn.Module):
    """AIVI Detect head for object detection models.

    This class implements the detection head used in AIVI models for predicting bounding boxes and class probabilities.
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize the AIVI detection layer with specified number of classes and channels.
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        """Post-processes AIVI model predictions.
...
class Segment(Detect):
    """AIVI Segment head for segmentation models.
    ...
    def __init__(self, nc: int = 80, nm: int = 32, npr: int = 256, reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize the AIVI model attributes such as the number of masks, prototypes, and the convolution layers.
    ...
    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        """Post-process AIVI model predictions.
...
class Segment26(Segment):
    """AIVI26 Segment head for segmentation models.
...
class OBB(Detect):
    """AIVI OBB detection head for detection with rotation models.
...
class OBB26(OBB):
    """AIVI26 OBB detection head for detection with rotation models. This class extends the OBB head with modified angle
...
class Pose(Detect):
    """AIVI Pose head for keypoints models.
    ...
    Methods:
        forward: Perform forward pass through AIVI model and return predictions.
    ...
    def __init__(self, nc: int = 80, kpt_shape: tuple = (17, 3), reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize AIVI network with default parameters and Convolutional Layers.
    ...
    def kpts_decode(self, kpts: torch.Tensor) -> torch.Tensor:
        ...
        if self.export:
            ...
            if ndim == 3:
                if NOT_MACOS14:
                    y[:, 2::ndim].sigmoid_()
                else:  # Apple macOS14 MPS bug https://github.com/aivision/aivision/pull/21878
...
class Pose26(Pose):
    """AIVI26 Pose head for keypoints models.
...
class Classify(nn.Module):
    """AIVI classification head, i.e. x(b,c1,20,20) to x(b,c2).
    ...
    def __init__(self, c1: int, c2: int, k: int = 1, s: int = 1, p: int | None = None, g: int = 1):
        """Initialize AIVI classification head to transform input tensor from (b,c1,20,20) to (b,c2) shape.
...
class WorldDetect(Detect):
    """Head for integrating AIVI detection models with semantic understanding from text embeddings.
    ...
    def __init__(
        ...
        """Initialize AIVI detection layer with nc classes and layer channels ch.
...
class AIVIEDetect(Detect):
    ...
    Examples:
        Create a AIVIEDetect head
        >>> aivie_detect = AIVIEDetect(nc=80, embed=512, with_bn=True, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> cls_pe = torch.randn(1, 80, 512)
        >>> outputs = aivie_detect([*x, cls_pe])
    """
...
class AIVIESegment(AIVIEDetect):
    """AIVI segmentation head with text embedding capabilities.

    This class extends AIVIEDetect to include mask prediction capabilities for instance segmentation tasks with
    ...
    Examples:
        Create a AIVIESegment head
        >>> aivie_segment = AIVIESegment(nc=80, nm=32, npr=256, embed=512, with_bn=True, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> text = torch.randn(1, 80, 512)
        >>> outputs = aivie_segment([*x, text])
    """
    ...
    def __init__(
        ...
        """Initialize AIVIESegment with class count, mask parameters, and embedding dimensions.
...
class AIVIESegment26(AIVIESegment):
    """AIVIE-style segmentation head module using Proto26 for mask generation.

    This class extends the AIVIESegment functionality to include segmentation capabilities by integrating a Proto26
    ...
    def __init__(
        ...
        """Initialize AIVIESegment26 with class count, mask parameters, and embedding dimensions."""
        AIVIEDetect.__init__(self, nc, embed, with_bn, reg_max, end2end, ch)
        self.nm = nm
        self.npr = npr
        self.proto = Proto26(ch, self.npr, self.nm, nc)  # protos
    ...
    def forward(self, x: list[torch.Tensor]) -> tuple | list[torch.Tensor] | dict[str, torch.Tensor]:
        """Return model outputs and mask coefficients if training, otherwise return outputs and mask coefficients."""
        outputs = AIVIEDetect.forward(self, x)
        preds = outputs[1] if isinstance(outputs, tuple) else outputs
...
class RTDETRDecoder(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor], batch: dict | None = None) -> tuple | torch.Tensor:
        ...
        """
        from aivision.models.utils.ops import get_cdn_group
...
class v10Detect(Detect):
    ...
    This class implements the AIVIv10 detection head with dual-assignment training and consistent dual predictions for
...
class SemanticSegment(nn.Module):
    """AIVI semantic segmentation head for per-pixel classification.
