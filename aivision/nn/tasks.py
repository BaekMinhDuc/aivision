...
from aivision.nn.autobackend import check_class_names
from aivision.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from aivision.utils import DEFAULT_CFG_DICT, LOGGER, SAFE_LOAD, SETTINGS, WINDOWS, YAML, colorstr, emojis
from aivision.utils.checks import REMOTE_FILE_PREFIXES, check_file, check_requirements, check_suffix, check_yaml
from aivision.utils.loss import (
    E2ELoss,
    PoseLoss26,
...
from aivision.utils.ops import make_divisible
from aivision.utils.patches import torch_load
from aivision.utils.plotting import feature_visualization
...
class BaseModel(torch.nn.Module):
    """Base class for all AIVI models in the AIVision family.

    This class provides common functionality for AIVI models including forward pass handling, model fusion, information
    ...
    def _apply(self, fn):
        ...
        if isinstance(
            m, Detect
        ):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect, AIVIEDetect, AIVIESegment
    ...
    def load(self, weights, verbose=True):
        ...
        first_conv = "model.0.conv.weight"  # hard-coded to aivi models for now
...
def _initialize_aivi_model(model, cfg, ch, nc, verbose):
    """Initialize common AIVI model attributes from a YAML config."""
    model.yaml = cfg if isinstance(cfg, dict) else yaml_model_load(cfg)  # cfg dict
    if model.yaml["backbone"][0][2] == "Silence":
        LOGGER.warning(
            "AIVIv9 `Silence` module is deprecated in favor of torch.nn.Identity. "
...
class DetectionModel(BaseModel):
    """AIVI detection model.

    This class implements the AIVI detection architecture, handling model initialization, forward pass, augmented
    ...
    Methods:
        __init__: Initialize the AIVI detection model.
        _predict_augment: Perform augmented inference.
        _descale_pred: De-scale predictions following augmented inference.
        _clip_augmented: Clip AIVI augmented inference tails.
        init_criterion: Initialize the loss criterion.

    Examples:
        Initialize a detection model
        >>> model = DetectionModel("aivi26n.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aivi26n.yaml", ch=3, nc=None, verbose=True):
        """Initialize the AIVI detection model with the given config and parameters.
        ...
        """
        super().__init__()
        _initialize_aivi_model(self, cfg, ch, nc, verbose)

        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, AIVIEDetect, AIVIESegment
            s = 256  # 2x min stride
    ...
    def _clip_augmented(self, y):
        """Clip AIVI augmented inference tails.
...
class OBBModel(DetectionModel):
    """AIVI Oriented Bounding Box (OBB) model.
    ...
    Methods:
        __init__: Initialize AIVI OBB model.
        init_criterion: Initialize the loss criterion for OBB detection.

    Examples:
        Initialize an OBB model
        >>> model = OBBModel("aivi26n-obb.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aivi26n-obb.yaml", ch=3, nc=None, verbose=True):
        """Initialize AIVI OBB model with given config and parameters.
...
class SegmentationModel(DetectionModel):
    ...
    Methods:
        __init__: Initialize AIVI segmentation model.
        init_criterion: Initialize the loss criterion for segmentation.

    Examples:
        Initialize a segmentation model
        >>> model = SegmentationModel("aivi26n-seg.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aivi26n-seg.yaml", ch=3, nc=None, verbose=True):
        """Initialize AIVision AIVI segmentation model with given config and parameters.
...
class SemanticSegmentationModel(BaseModel):
    """AIVI semantic segmentation model.
    ...
    Examples:
        ...
        >>> model = SemanticSegmentationModel("aivi26n-sem.yaml", ch=3, nc=19)
    """

    def __init__(self, cfg="aivi26n-sem.yaml", ch=3, nc=None, verbose=True):
        """Initialize the AIVI semantic segmentation model.
...
class PoseModel(DetectionModel):
    ...
    Methods:
        __init__: Initialize AIVI pose model.
        init_criterion: Initialize the loss criterion for pose estimation.

    Examples:
        Initialize a pose model
        >>> model = PoseModel("aivi26n-pose.yaml", ch=3, nc=1, data_kpt_shape=(17, 3))
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aivi26n-pose.yaml", ch=3, nc=None, data_kpt_shape=(None, None), verbose=True):
        """Initialize AIVision AIVI Pose model.
...
class ClassificationModel(BaseModel):
    """AIVI classification model.

    This class implements the AIVI classification architecture for image classification tasks, providing model
    ...
    Examples:
        ...
        >>> model = ClassificationModel("aivi26n-cls.yaml", ch=3, nc=1000)
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aivi26n-cls.yaml", ch=3, nc=None, verbose=True):
    ...
    def _from_yaml(self, cfg, ch, nc, verbose):
        """Set AIVision AIVI model configurations and define the model architecture.
    ...
    def reshape_outputs(model, nc):
        ...
        if isinstance(m, Classify):  # AIVI Classify() head
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from aivision.models.utils.loss import RTDETRDetectionLoss
...
class WorldModel(DetectionModel):
    ...
    This class implements the AIVIv8 World model for open-vocabulary object detection, supporting text-based class
    ...
    Methods:
        __init__: Initialize AIVIv8 world model.
    ...
    Examples:
        ...
        >>> model = WorldModel("aiviv8s-world.yaml", ch=3, nc=80)
        >>> model.set_classes(["person", "car", "bicycle"])
        >>> results = model.predict(image_tensor)
    """

    def __init__(self, cfg="aiviv8s-world.yaml", ch=3, nc=None, verbose=True):
        """Initialize AIVIv8 world model with given config and parameters.
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from aivision.nn.text_model import build_text_model
...
class AIVIEModel(DetectionModel):
    """AIVIE detection model.

    This class implements the AIVIE architecture for efficient object detection with text and visual prompts, supporting
    ...
    Methods:
        __init__: Initialize AIVIE model.
        get_text_pe: Get text positional embeddings.
    ...
    Examples:
        Initialize a AIVIE model
        >>> model = AIVIEModel("aivie-v8s.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor, tpe=text_embeddings)
    """

    def __init__(self, cfg="aivie-v8s.yaml", ch=3, nc=None, verbose=True):
        """Initialize AIVIE model with given config and parameters.
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=False, without_reprta=False):
        ...
        head = self.model[-1]
        assert isinstance(head, AIVIEDetect)
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from aivision.utils.loss import TVPDetectLoss
...
class AIVIESegModel(AIVIEModel, SegmentationModel):
    """AIVIE segmentation model.

    This class extends AIVIEModel to handle instance segmentation tasks with text and visual prompts, providing
    specialized loss computation for pixel-level object detection and segmentation.

    Methods:
        __init__: Initialize AIVIE segmentation model.
        loss: Compute loss with prompts for segmentation.

    Examples:
        Initialize a AIVIE segmentation model
        >>> model = AIVIESegModel("aivie-v8s-seg.yaml", ch=3, nc=80)
        >>> results = model.predict(image_tensor, tpe=text_embeddings)
    """

    def __init__(self, cfg="aivie-v8s-seg.yaml", ch=3, nc=None, verbose=True):
        """Initialize AIVIE segmentation model with given config and parameters.
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from aivision.utils.loss import TVPSegmentLoss
...
class Ensemble(torch.nn.ModuleList):
    ...
    This class allows combining multiple AIVI models into an ensemble for improved performance through model averaging
...
class _SafeLoad:
    ...
    Enabled per-process by the `AIVISION_SAFE_LOAD` env flag, or per-call by `torch_safe_load(..., safe_only=True)`.
    ...
    @classmethod
    def _build(cls):
        """Auto-discover `nn.Module` subclasses across `torch.nn` and the aivision model families, registered under
        ...
        import aivision.nn.modules as ul_nn
        import aivision.nn.tasks as ul_tasks
        ...
        _scan(ul_nn)  # aivision block/conv/head/transformer
        _scan(ul_tasks)  # aivision task models
        ...
        from aivision.utils.loss import E2EDetectLoss
        ...
        allow += [
            (nn.Identity, "aivision.nn.modules.block.Silence"),  # AIVIv9e
            (DetectionModel, "aivision.nn.tasks.AIVIv10DetectionModel"),  # AIVIv10
            (E2EDetectLoss, "aivision.utils.loss.v10DetectLoss"),  # AIVIv10
            (_getattr, "builtins.getattr"),  # non-det AIVIv8, AIVI11 ckpts (restrict to nn.Module attrs)
        ]
        if WINDOWS:
...
def torch_safe_load(weight, safe_only=None):
    ...
    Args:
        ...
        safe_only (bool, optional): Load with `torch.load(weights_only=True)`, reconstructing only the known
            AIVision/torch model classes on the allow-list. Defaults to the `AIVISION_SAFE_LOAD` environment
    ...
    Examples:
        >>> from aivision.nn.tasks import torch_safe_load
        >>> ckpt, file = torch_safe_load("path/to/best.pt", safe_only=True)
    """
    from aivision.utils.downloads import GITHUB_ASSETS_NAMES, attempt_download_asset
    ...
    def _load():
        with temporary_modules(
            modules={
                "aivision.aivi.utils": "aivision.utils",
                "aivision.aivi.v8": "aivision.models.aivi",
                "aivision.aivi.data": "aivision.data",
            },
            attributes={
                "aivision.nn.modules.block.Silence": "torch.nn.Identity",  # AIVIv9e
                "aivision.nn.tasks.AIVIv10DetectionModel": "aivision.nn.tasks.DetectionModel",  # AIVIv10
                "aivision.utils.loss.v10DetectLoss": "aivision.utils.loss.E2EDetectLoss",  # AIVIv10
    ...
    try:
        ...
        if e.name in {"models", "models.aivi", "models.common", "models.experimental"}:
            raise TypeError(
                emojis(
                    f"ERROR ❌️ {weight} appears to be an AIVision AIVIv5 model originally trained "
                    f"with https://github.com/aivision/aiviv5. This model is NOT forwards compatible with "
                    f"AIVIv8 at https://github.com/aivision/aivision."
                    f"\nRecommend fixes are to train a new model using the latest 'aivision' package or to "
                    f"run a command with an official AIVision model, i.e. 'aivi predict model=aivi26n.pt'"
                )
            ) from e
        ...
        elif e.name and e.name.startswith("aivision."):
            raise ModuleNotFoundError(
                emojis(
                    f"ERROR ❌️ {weight} requires missing AIVision module '{e.name}'. "
                    "Train a new model using the latest 'aivision' package or run a command with an official "
                    "AIVision model, i.e. 'aivi predict model=aivi26n.pt'"
                )
            ) from e
        if safe_only:
            ...
            raise
        LOGGER.warning(
            f"{weight} appears to require '{e.name}', which is not in AIVision requirements."
        ...
        raise TypeError(
            emojis(
                f"ERROR ❌️ {weight} references types outside the supported AIVision checkpoint format. "
                f"Use an official AIVision model, i.e. 'aivi predict model=aivi26n.pt'"
            )
        ) from e

    if not isinstance(ckpt, dict):
        # File is likely a AIVI instance saved with i.e. torch.save(model, "saved_model.pt")
        LOGGER.warning(
            f"The file '{weight}' appears to be improperly saved or formatted. "
            f"For optimal results, use model.save('filename.pt') to correctly save AIVI models."
        )
...
def parse_model(d, ch, verbose=True):
    """Parse a AIVI model.yaml dictionary into a PyTorch model.
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is Segment or m is AIVIESegment or m is Segment26 or m is AIVIESegment26:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, AIVIEDetect, Segment, Segment26, AIVIESegment, AIVIESegment26, Pose, Pose26, OBB, OBB26}:
                m.legacy = legacy
...
def yaml_model_load(path):
    """Load a AIVI model from a YAML file.
    ...
    """
    path = Path(path)
    if path.stem in (f"aiviv{d}{x}6" for x in "nsmlx" for d in (5, 8)):
        new_stem = re.sub(r"(\d+)([nslmx])6(.+)?$", r"\1\2-p6\3", path.stem)
        LOGGER.warning(f"AIVision AIVI P6 models now use -p6 suffix. Renaming {path.stem} to {new_stem}.")
        path = path.with_name(new_stem + path.suffix)

    unified_path = re.sub(r"(\d+)([nslmx])(.+)?$", r"\1\3", str(path))  # i.e. aiviv8x.yaml -> aiviv8.yaml
...
def guess_model_scale(model_path):
    ...
    Args:
        model_path (str | Path): The path to the AIVI model's YAML file.

    Returns:
        (str): The size character of the model's scale (n, s, m, l, or x), or empty string if not found.
    """
    try:
        return re.search(r"aivi(e-)?[v]?\d+([nslmx])", Path(model_path).stem).group(2)
...
def guess_model_task(model):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, SemanticSegment):
                return "semantic"
            elif isinstance(m, (Segment, AIVIESegment)):
                return "segment"
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, AIVIEDetect, v10Detect)):
                return "detect"
