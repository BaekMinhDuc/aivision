...
from aivision.hub.auth import Auth
from aivision.hub.session import HUBTrainingSession
from aivision.hub.utils import HUB_API_ROOT, HUB_WEB_ROOT, PREFIX
from aivision.utils import LOGGER, SETTINGS, checks
...
def login(api_key: str | None = None, save: bool = True) -> bool:
    """Log in to the AIVision HUB API using the provided API key.
    ...
    if client.authenticated:
        ...
        LOGGER.info(f"{PREFIX}Get API key from {api_key_url} and then run 'aivi login API_KEY'")
        return False


def logout():
    """Log out of AIVision HUB by removing the API key from the settings file."""
    SETTINGS["api_key"] = ""
    LOGGER.info(f"{PREFIX}logged out ✅. To log in again, use 'aivi login'.")
...
def export_model(model_id: str = "", format: str = "torchscript"):
    """Export a model to a specified format for deployment via the AIVision HUB API.
...
def get_export(model_id: str = "", format: str = "torchscript"):
    """Retrieve an exported model in the specified format from AIVision HUB using the model ID.

    Args:
        model_id (str): The ID of the model to retrieve from AIVision HUB.
...
def check_dataset(path: str, task: str) -> None:
    ...
    Examples:
        >>> from aivision.hub import check_dataset
