...
from aivision.hub.utils import HELP_MSG, HUB_WEB_ROOT, PREFIX
from aivision.utils import IS_COLAB, LOGGER, SETTINGS, TQDM, checks
from aivision.utils.errors import HUBModelError

AGENT_NAME = f"python-{__version__}-colab" if IS_COLAB else f"python-{__version__}-local"


class HUBTrainingSession:
    """HUB training session for AIVision HUB AIVI models.

    This class encapsulates the functionality for interacting with AIVision HUB during model training, including
    model creation, metrics tracking, and checkpoint uploading.

    Attributes:
        model_id (str): Identifier for the AIVI model being trained.
        model_url (str): URL for the model in AIVision HUB.
        ...
        model (Any): Model data fetched from AIVision HUB.
        model_file (str): Path to the model file.
        train_args (dict[str, Any]): Arguments for training the model.
        client (Any): Client for interacting with AIVision HUB.
        filename (str): Filename of the model.

    Examples:
        Create a training session with a model URL
        >>> session = HUBTrainingSession("https://hub.aivision.com/models/example-model")
    ...
    def __init__(self, identifier: str):
        ...
        self.timers = {}  # holds timers in aivision/utils/callbacks/hub.py
        self.model = None
        self.model_url = None
        ...
        try:
            ...
            if identifier.startswith(f"{HUB_WEB_ROOT}/models/") and not self.client.authenticated:
                LOGGER.warning(
                    f"{PREFIX}Please log in using 'aivi login API_KEY'. "
                    "You can find your API Key at: https://hub.aivision.com/settings?tab=api+keys."
                )
    ...
    def load_model(self, model_id: str):
        """Load an existing model from AIVision HUB using the provided model identifier.
    ...
    def create_model(self, model_args: dict[str, Any]):
        ...
        if not self.model.id:
            raise HUBModelError(f"❌ Failed to create model '{self.filename}' on AIVision HUB. Please try again.")
    ...
    def _parse_identifier(identifier: str):
        ...
        The method supports different identifier formats:
            - A HUB model URL https://hub.aivision.com/models/MODEL
            - A HUB model URL with API Key https://hub.aivision.com/models/MODEL?api_key=APIKEY
    ...
    def _set_train_args(self):
        """Initialize training arguments and create a model entry on the AIVision HUB.
        ...
        self.model_file = checks.check_aiviv5u_filename(self.model_file, verbose=False)  # AIVIv5->AIVIv5u
    ...
    def upload_metrics(self):
        """Upload model metrics to AIVision HUB."""
    ...
    def upload_model(
        ...
        """Upload a model checkpoint to AIVision HUB.
        ...
        if not weights.is_file():
            ...
            if final and last.is_file():
                LOGGER.warning(
                    ...
                    "For more reliable training, consider using AIVision HUB Cloud. "
                    "Learn more at https://docs.aivision.com/platform."
                )
