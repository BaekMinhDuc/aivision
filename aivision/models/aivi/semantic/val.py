...
from aivision.data.dataset import SemanticDataset
from aivision.data.utils import add_polygon_background
from aivision.models.aivi.detect import DetectionValidator
from aivision.utils import LOGGER, RANK
from aivision.utils.metrics import ConfusionMatrix, SemanticMetrics
...
class SemanticSegmentationValidator(DetectionValidator):
    ...
    Examples:
        >>> from aivision.models.aivi.semantic import SemanticSegmentationValidator
        >>> args = dict(model="aivi26n-sem.pt", data="cityscapes8.yaml")
    ...
    def postprocess(self, preds):
        ...
        if preds.ndim == 3:  # exports with in-graph ArgMax emit [B, H, W] class maps directly
            if preds.shape[1:] != tuple(self._semantic_target_shape):
                preds = F.interpolate(
                    preds.unsqueeze(1).float(), size=self._semantic_target_shape, mode="nearest"
                ).squeeze(1)
