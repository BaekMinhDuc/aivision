...
from aivision.models.aivi.detect import DetectionPredictor
from aivision.models.aivi.segment import SegmentationPredictor


class AIVIEVPDetectPredictor(DetectionPredictor):
    """A class extending DetectionPredictor for AIVI-EVP (Enhanced Visual Prompting) predictions.

    This class provides common functionality for AIVI models that use visual prompting, including model setup, prompt
    handling, and preprocessing transformations.

    Attributes:
        model (torch.nn.Module): The AIVI model for inference.
        device (torch.device): Device to run the model on (CPU or CUDA).
        prompts (dict | torch.Tensor): Visual prompts containing class indices and bounding boxes or masks.

    Methods:
        setup_model: Initialize the AIVI model and set it to evaluation mode.
...
class AIVIEVPSegPredictor(AIVIEVPDetectPredictor, SegmentationPredictor):
    """Predictor for AIVI-EVP segmentation tasks combining detection and segmentation capabilities."""
