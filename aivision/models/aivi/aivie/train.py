...
from aivision.data import AIVIConcatDataset, build_aivi_dataset
from aivision.data.augment import LoadVisualPrompt
from aivision.models.aivi.detect import DetectionTrainer, DetectionValidator
from aivision.nn.tasks import AIVIEModel
from aivision.utils import DEFAULT_CFG, LOGGER, RANK
from aivision.utils.torch_utils import unwrap_model

from ..world.train_world import WorldTrainerFromScratch
from .val import AIVIEDetectValidator


class AIVIETrainer(DetectionTrainer):
    """A trainer class for AIVIE object detection models.

    This class extends DetectionTrainer to provide specialized training functionality for AIVIE models, including custom
    ...
    Methods:
        get_model: Initialize and return a AIVIEModel with specified configuration.
        get_validator: Return a AIVIEDetectValidator for model validation.
        build_dataset: Build AIVI dataset with multi-modal support for training.
    """

    def __init__(self, cfg=DEFAULT_CFG, overrides: dict | None = None, _callbacks: dict | None = None):
        """Initialize the AIVIE Trainer with specified configurations.
    ...
    def get_model(self, cfg=None, weights=None, verbose: bool = True):
        """Return a AIVIEModel initialized with the specified configuration and weights.
        ...
        Returns:
            (AIVIEModel): The initialized AIVIE model.
    ...
    def get_validator(self):
        """Return a AIVIEDetectValidator for AIVIE model validation."""
        self.loss_names = "box", "cls", "dfl"
        return AIVIEDetectValidator(
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch: int | None = None):
        ...
        Returns:
            (Dataset): AIVI dataset configured for training or validation.
        """
        gs = max(int(unwrap_model(self.model).stride.max() if self.model else 0), 32)
        return build_aivi_dataset(
            self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs, multi_modal=mode == "train"
        )


class AIVIEPETrainer(DetectionTrainer):
    """Fine-tune AIVIE model using linear probing approach.
    ...
    Methods:
        get_model: Initialize AIVIEModel with frozen layers except projection layers.
    """

    def get_model(self, cfg=None, weights=None, verbose: bool = True):
        """Return AIVIEModel initialized with specified config and weights.
        ...
        Returns:
            (AIVIEModel): Initialized model with frozen layers except for specific projection layers.
        """
        model = AIVIEModel(
            cfg["yaml_file"] if isinstance(cfg, dict) else cfg,
            ch=self.data["channels"],
        ...
        # NOTE: `get_text_pe` related to text model and AIVIEDetect.reprta,
...
class AIVIETrainerFromScratch(AIVIETrainer, WorldTrainerFromScratch):
    """Train AIVIE models from scratch with text embedding support.

    This trainer combines AIVIE training capabilities with world training features, enabling training from scratch with
    ...
    def build_dataset(self, img_path: list[str] | str, mode: str = "train", batch: int | None = None):
        """Build AIVI Dataset for training or validation.

        This method constructs appropriate datasets based on the mode and input paths, handling both standard AIVI
        ...
        Returns:
            (AIVIConcatDataset | Dataset): The constructed dataset for training or validation.
        """
...
class AIVIEPEFreeTrainer(AIVIEPETrainer, AIVIETrainerFromScratch):
    """Train prompt-free AIVIE model.

    This trainer combines linear probing capabilities with from-scratch training for prompt-free AIVIE models that don't
    ...
    def get_validator(self):
        """Return a DetectionValidator for AIVI model validation."""
        self.loss_names = "box", "cls", "dfl"
    ...
    def preprocess_batch(self, batch):
        """Preprocess a batch of images for AIVIE training, adjusting formatting and dimensions as needed."""
...
class AIVIEVPTrainer(AIVIETrainerFromScratch):
    """Train AIVIE model with visual prompts.

    This trainer extends AIVIETrainerFromScratch to support visual prompt-based training, where visual cues are provided
    ...
    def build_dataset(self, img_path: list[str] | str, mode: str = "train", batch: int | None = None):
        """Build AIVI Dataset for training or validation with visual prompts.
        ...
        Returns:
            (AIVIConcatDataset | Dataset): AIVI dataset configured for training or validation, with visual prompts for
                training mode.
        """
        dataset = super().build_dataset(img_path, mode, batch)
        if isinstance(dataset, AIVIConcatDataset):
    ...
    def _close_dataloader_mosaic(self):
        ...
        if isinstance(self.train_loader.dataset, AIVIConcatDataset):
