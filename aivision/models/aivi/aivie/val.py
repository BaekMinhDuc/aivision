...
from aivision.data import AIVIConcatDataset, build_dataloader, build_aivi_dataset
from aivision.data.augment import LoadVisualPrompt
from aivision.data.utils import check_det_dataset
from aivision.models.aivi.detect import DetectionValidator
from aivision.models.aivi.segment import SegmentationValidator
from aivision.nn.modules.head import AIVIEDetect
from aivision.nn.tasks import AIVIEModel
from aivision.utils import LOGGER, TQDM
from aivision.utils.torch_utils import select_device, smart_inference_mode


class AIVIEDetectValidator(DetectionValidator):
    """A validator class for AIVIE detection models that handles both text and visual prompt embeddings.

    This class extends DetectionValidator to provide specialized validation functionality for AIVIE models. It supports
    ...
    Examples:
        ...
        >>> validator = AIVIEDetectValidator()
    ...
    def get_visual_pe(self, dataloader: torch.utils.data.DataLoader, model: AIVIEModel) -> torch.Tensor:
        """Extract visual prompt embeddings from training samples.

        This method processes a dataloader to compute visual prompt embeddings for each class using a AIVIE model. It
        ...
        Args:
            ...
            model (AIVIEModel): The AIVIE model from which to extract visual prompt embeddings.

        Returns:
            (torch.Tensor): Visual prompt embeddings with shape (1, num_classes, embed_dim).
        """
        assert isinstance(model, AIVIEModel)
    ...
    def __call__(
        ...
        model: AIVIEModel | str | None = None,
        ...
        Args:
            ...
            model (AIVIEModel | str, optional): Model to validate. Required if trainer is not provided.
        ...
        if trainer is not None:
            ...
            elif isinstance(model.model[-1], AIVIEDetect) and hasattr(model.model[-1], "lrpc"):  # prompt-free
...
class AIVIESegValidator(AIVIEDetectValidator, SegmentationValidator):
    """AIVIE segmentation validator that supports both text and visual prompt embeddings."""
