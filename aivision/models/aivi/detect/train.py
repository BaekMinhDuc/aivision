...
from aivision.engine.trainer import BaseTrainer
...
from aivision.utils.patches import override_configs
from aivision.utils.plotting import plot_images, plot_labels
from aivision.utils.torch_utils import torch_distributed_zero_first, unwrap_model


class DetectionTrainer(BaseTrainer):
    """A class extending the BaseTrainer class for training based on a detection model.

    This trainer specializes in object detection tasks, handling the specific requirements for training AIVI models for
    object detection including dataset building, data loading, preprocessing, and model configuration.

    Attributes:
        model (DetectionModel): The AIVI detection model being trained.
        data (dict): Dictionary containing dataset information including class names and number of classes.
        loss_names (tuple): Names of the loss components used in training (box_loss, cls_loss, dfl_loss).

    Methods:
        build_dataset: Build AIVI dataset for training or validation.
        get_dataloader: Construct and return dataloader for the specified mode.
        preprocess_batch: Preprocess a batch of images by scaling and converting to float.
        set_model_attributes: Set model attributes based on dataset information.
        get_model: Return a AIVI detection model.
        get_validator: Return a validator for model evaluation.
        label_loss_items: Return a loss dictionary with labeled training loss items.
        progress_string: Return a formatted string of training progress.
        plot_training_samples: Plot training samples with their annotations.
        plot_training_labels: Create a labeled training plot of the AIVI model.
        auto_batch: Calculate optimal batch size based on model memory requirements.

    Examples:
        >>> from aivision.models.aivi.detect import DetectionTrainer
        >>> args = dict(model="aivi26n.pt", data="coco8.yaml", epochs=3)
        >>> trainer = DetectionTrainer(overrides=args)
        >>> trainer.train()
    """

    def __init__(self, cfg=DEFAULT_CFG, overrides: dict[str, Any] | None = None, _callbacks: dict | None = None):
        """Initialize a DetectionTrainer object for training AIVI object detection models.
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch: int | None = None):
        ...
        Returns:
            (Dataset): AIVI dataset object configured for the specified mode.
        """
        gs = max(int(unwrap_model(self.model).stride.max()), 32)
        return build_aivi_dataset(self.args, img_path, batch, self.data, mode=mode, rect=mode == "val", stride=gs)
    ...
    def get_model(self, cfg: str | None = None, weights: str | None = None, verbose: bool = True):
        ...
        Returns:
            (DetectionModel): AIVI detection model.
        """
    ...
    def get_validator(self):
        ...
        return aivi.detect.DetectionValidator(
    ...
    def plot_training_labels(self):
        """Create a labeled training plot of the AIVI model."""
