...
from aivision.nn.tasks import OBBModel
from aivision.utils import DEFAULT_CFG, RANK


class OBBTrainer(aivi.detect.DetectionTrainer):
    """A class extending the DetectionTrainer class for training based on an Oriented Bounding Box (OBB) model.

    This trainer specializes in training AIVI models that detect oriented bounding boxes, which are useful for detecting
    ...
    Methods:
        ...
        get_validator: Return an instance of OBBValidator for validation of AIVI model.

    Examples:
        >>> from aivision.models.aivi.obb import OBBTrainer
        >>> args = dict(model="aivi26n-obb.pt", data="dota8.yaml", epochs=3)
    ...
    def get_model(
        ...
        Examples:
            >>> trainer = OBBTrainer()
            >>> model = trainer.get_model(cfg="aivi26n-obb.yaml", weights="aivi26n-obb.pt")
        """
    ...
    def get_validator(self):
        """Return an instance of OBBValidator for validation of AIVI model."""
        self.loss_names = "box_loss", "cls_loss", "dfl_loss", "angle_loss"
        return aivi.obb.OBBValidator(
