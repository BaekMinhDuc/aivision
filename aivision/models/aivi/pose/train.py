...
from aivision.nn.tasks import PoseModel
from aivision.utils import DEFAULT_CFG, RANK
from aivision.utils.torch_utils import unwrap_model


class PoseTrainer(aivi.detect.DetectionTrainer):
    """A class extending the DetectionTrainer class for training AIVI pose estimation models.
    ...
    Examples:
        >>> from aivision.models.aivi.pose import PoseTrainer
        >>> args = dict(model="aivi26n-pose.pt", data="coco8-pose.yaml", epochs=3)
        >>> trainer = PoseTrainer(overrides=args)
        >>> trainer.train()
    """

    def __init__(self, cfg=DEFAULT_CFG, overrides: dict[str, Any] | None = None, _callbacks: dict | None = None):
        """Initialize a PoseTrainer object for training AIVI pose estimation models.
    ...
    def get_validator(self):
        ...
        return aivi.pose.PoseValidator(
    ...
    def get_dataset(self) -> dict[str, Any]:
        ...
        if "kpt_shape" not in data:
            raise KeyError(f"No `kpt_shape` in the {self.args.data}. See https://docs.aivision.com/datasets/pose/")
        return data
