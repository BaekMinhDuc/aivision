...
from aivision.utils import ops
from aivision.utils.metrics import OKS_SIGMA, PoseMetrics, kpt_iou
...
class PoseValidator(DetectionValidator):
    ...
    Methods:
        ...
        init_metrics: Initialize pose estimation metrics for AIVI model.
        postprocess: Postprocess AIVI predictions to extract and reshape keypoints for pose estimation.
        ...
        save_one_txt: Save AIVI pose detections to a text file in normalized coordinates.
        pred_to_json: Convert AIVI predictions to COCO JSON format.
        eval_json: Evaluate object detection model using COCO JSON format.

    Examples:
        >>> from aivision.models.aivi.pose import PoseValidator
        >>> args = dict(model="aivi26n-pose.pt", data="coco8-pose.yaml")
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        """Initialize evaluation metrics for AIVI pose validation.
    ...
    def postprocess(self, preds: torch.Tensor) -> list[dict[str, torch.Tensor]]:
        """Postprocess AIVI predictions to extract and reshape keypoints for pose estimation.
        ...
        Args:
            preds (torch.Tensor): Raw prediction tensor from the AIVI pose model containing bounding boxes, confidence
    ...
    def save_one_txt(self, predn: dict[str, torch.Tensor], save_conf: bool, shape: tuple[int, int], file: Path) -> None:
        """Save AIVI pose detections to a text file in normalized coordinates.
    ...
    def pred_to_json(self, predn: dict[str, torch.Tensor], pbatch: dict[str, Any]) -> None:
        """Convert AIVI predictions to COCO JSON format.

        This method takes prediction tensors and batch data, converts the bounding boxes from AIVI format to COCO
