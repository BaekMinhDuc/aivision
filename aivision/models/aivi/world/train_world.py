...
from aivision.data import AIVIConcatDataset, build_grounding, build_aivi_dataset
from aivision.data.utils import check_det_dataset
from aivision.models.aivi.world import WorldTrainer
from aivision.utils import DATASETS_DIR, DEFAULT_CFG, LOGGER
...
class WorldTrainerFromScratch(WorldTrainer):
    ...
    supporting training AIVI-World models with combined vision-language capabilities.
    ...
    Methods:
        build_dataset: Build AIVI Dataset for training or validation with mixed dataset support.
        get_dataset: Get train and validation paths from data dictionary.
        plot_training_labels: Skip label plotting for AIVI-World training.
        final_eval: Perform final evaluation and validation for the AIVI-World model.

    Examples:
        >>> from aivision.models.aivi.world.train_world import WorldTrainerFromScratch
        >>> from aivision import AIVIWorld
        >>> data = dict(
        ...     train=dict(
        ...         aivi_data=["Objects365.yaml"],
        ...         grounding_data=[
        ...             dict(
        ...
        ...         ],
        ...     ),
        ...     val=dict(aivi_data=["lvis.yaml"]),
        ... )
        >>> model = AIVIWorld("aiviv8s-worldv2.yaml")
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks: dict | None = None):
        ...
        This initializes a trainer for AIVI-World models from scratch, supporting mixed datasets including both object
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        if mode != "train":
            return build_aivi_dataset(self.args, img_path, batch, self.data, mode=mode, rect=False, stride=gs)
        datasets = [
            build_aivi_dataset(self.args, im_path, batch, self.training_data[im_path], stride=gs, multi_modal=True)
        ...
        return AIVIConcatDataset(datasets) if len(datasets) > 1 else datasets[0]
    ...
    def check_data_config(data: dict | str | Path) -> dict:
        ...
        if not isinstance(data, dict):
            from aivision.utils import YAML
    ...
    def get_dataset(self):
        ...
        Processes the data configuration to extract paths for training and validation datasets, handling both AIVI
        ...
        data = {k: [check_det_dataset(d) for d in v.get("aivi_data", [])] for k, v in data_yaml.items()}
    ...
    def plot_training_labels(self):
        """Skip label plotting for AIVI-World training."""
        pass

    def final_eval(self):
        """Perform final evaluation and validation for the AIVI-World model.
        ...
        """
        val = self.args.data["val"]["aivi_data"][0]
