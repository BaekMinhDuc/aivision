...
class NASPredictor(DetectionPredictor):
    """AIVision AIVI NAS Predictor for object detection.

    This class extends the DetectionPredictor from AIVision engine and is responsible for post-processing the raw
    predictions generated by the AIVI NAS models. It applies operations like non-maximum suppression and scaling the
    ...
    Attributes:
        ...
        model (torch.nn.Module): The AIVI NAS model used for inference.
    ...
    def postprocess(self, preds_in, img, orig_imgs):
        ...
        This method takes raw predictions from a AIVI NAS model, converts bounding box formats, and applies
        post-processing operations to generate the final detection results compatible with AIVision result
        ...
        Examples:
            >>> predictor = NAS("aivi_nas_s").predictor
            >>> results = predictor.postprocess(raw_preds, img, orig_imgs)
        """
