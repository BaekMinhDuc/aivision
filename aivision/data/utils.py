...
from aivision.utils.checks import check_file, check_font, is_ascii
from aivision.utils.downloads import download, safe_download, unzip_file
from aivision.utils.ops import segments2boxes

HELP_URL = "See https://docs.aivision.com/datasets for dataset formatting guidance."
IMG_FORMATS = {
    "avif",
    "bmp",
    "dng",
...
def check_file_speeds(
    ...
    if avg_ping < threshold_ms or avg_speed < threshold_mb:
        ...
        LOGGER.warning(
            ...
            f"See https://docs.aivision.com/guides/model-training-tips/"
        )
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    """Visualize AIVI annotations (bounding boxes and class labels) on an image.

    This function reads an image and its corresponding annotation file in AIVI format, then draws bounding boxes around
    ...
    Args:
        ...
        txt_path (str): Path to the annotation file in AIVI format, which should contain one line per object.
    ...
    from aivision.utils.plotting import colors
...
def convert_ndjson_to_aivi_if_needed(data: str | Path) -> str | Path:
    """Convert an NDJSON dataset or Platform dataset URI to AIVI format."""
    data_str = str(data)
    if data_str.endswith(".ndjson") or (data_str.startswith("ul://") and "/datasets/" in data_str):
        import asyncio

        from aivision.data.converter import convert_ndjson_to_aivi

        return asyncio.run(convert_ndjson_to_aivi(check_file(data)))
    return data
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    for k in "train", "val":
        if k not in data:
            ...
            LOGGER.warning("renaming data YAML 'validation' key to 'val' to match AIVI format.")
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not data_dir.is_dir():
        if data_dir.suffix != "":
            raise ValueError(
                ...
                "See https://docs.aivision.com/datasets/classify/"
            )
        LOGGER.info("")
    ...
    if not train_set.is_dir():
        ...
        if image_files := list(data_dir.rglob("*.jpg")) + list(data_dir.rglob("*.png")):
            from aivision.data.split import split_classify_dataset
...
class HUBDatasetStats:
    ...
    Methods:
        get_json: Return dataset JSON for AIVision HUB.
        process_images: Compress images for AIVision HUB.

    Examples:
        >>> from aivision.data.utils import HUBDatasetStats
    ...
    Notes:
        Download *.zip files from https://github.com/aivision/hub/tree/main/example_datasets
        i.e. https://github.com/aivision/hub/raw/main/example_datasets/coco8.zip for coco8.zip.
    """
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        """Return dataset JSON for AIVision HUB."""
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                from torchvision.datasets import ImageFolder  # scope for faster 'import aivision'
                ...
                dataset = AIVIDataset(img_path=self.data[split], data=self.data, task=self.task)
                x = np.array(
                    [
    ...
    def process_images(self) -> Path:
        """Compress images for AIVision HUB."""
        from aivision.data import AIVIDataset  # ClassificationDataset

        self.im_dir.mkdir(parents=True, exist_ok=True)  # makes dataset-hub/images/
        for split in "train", "val", "test":
            if self.data.get(split) is None:
                continue
            dataset = AIVIDataset(img_path=self.data[split], data=self.data)
...
def compress_one_image(f: str, f_new: str | None = None, max_dim: int = 1920, quality: int = 50):
    ...
    Examples:
        >>> from pathlib import Path
        >>> from aivision.data.utils import compress_one_image
...
def load_dataset_cache_file(path: Path) -> dict:
    """Load an AIVision *.cache dictionary from path."""
    import gc

    gc.disable()  # reduce pickle load time https://github.com/aivision/aivision/pull/1585
...
def save_dataset_cache_file(prefix: str, path: Path, x: dict, version: str):
    """Save an AIVision dataset *.cache dictionary x to path."""
