...
from aivision.data.utils import polygons2masks, polygons2masks_overlap
from aivision.utils import LOGGER, IterableSimpleNamespace, colorstr
from aivision.utils.checks import check_version
from aivision.utils.instance import Instances
from aivision.utils.metrics import bbox_ioa
from aivision.utils.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
from aivision.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class BaseTransform:
    """Base class for image transformations in the AIVision library.
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from aivision.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from aivision.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p: float = 0.0) -> None:
        ...
        and labels. This implementation is designed for use with the AIVision AIVI framework.
...
class CutMix(BaseMixTransform):
    ...
    Examples:
        >>> from aivision.data.augment import CutMix
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV(BaseTransform):
    ...
    Examples:
        >>> import numpy as np
        >>> from aivision.data.augment import RandomHSV
    ...
    def apply_image(self, labels, params: dict[str, Any] | None = None):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # lut_hue = ((x * (r[0] + 1)) % 180).astype(dtype)   # original hue implementation from aivision<=8.3.78
...
class LetterBox(BaseTransform):
    ...
    def __init__(
        ...
        Args:
            ...
            stride (int): Stride of the model (e.g., 32 for AIVIv5).
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from aivision.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
class Albumentations(BaseTransform):
    ...
    def __init__(self, p: float = 1.0, transforms: list | None = None) -> None:
        """Initialize the Albumentations transform object for AIVI bbox formatted parameters.
        ...
        try:
            ...
            self.transform = (
                A.Compose(T, bbox_params=A.BboxParams(format="aivi", label_fields=["class_labels"]))
...
class Format(BaseTransform):
    ...
    def _format_img(self, img: np.ndarray) -> torch.Tensor:
        """Format an image for AIVI from a Numpy array to a PyTorch tensor.
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    This function creates a composition of image augmentation techniques to prepare images for AIVI training. It
    ...
    Examples:
        >>> from aivision.cfg import DEFAULT_CFG
        >>> from aivision.data.dataset import AIVIDataset
        >>> from aivision.utils import IterableSimpleNamespace
        >>> dataset = AIVIDataset(img_path="path/to/images", data={"names": {0: "person"}}, imgsz=640)
...
def classify_transforms(
    ...
    """
    import torchvision.transforms as T  # scope for faster 'import aivision'
...
class ToTensor:
    ...
    def __init__(self, half: bool = False):
        ...
        AIVision AIVI framework. It converts numpy arrays or PIL Images to PyTorch tensors, with an option for
