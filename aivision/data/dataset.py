...
from aivision.utils import LOCAL_RANK, LOGGER, NUM_THREADS, TQDM, colorstr
from aivision.utils.instance import Instances
from aivision.utils.ops import resample_segments, segments2boxes
from aivision.utils.torch_utils import TORCHVISION_0_18
...
# AIVision dataset *.cache version, >= 1.0.0 for AIVision AIVI models
DATASET_CACHE_VERSION = "1.0.3"


class AIVIDataset(BaseDataset):
    """Dataset class for loading object detection and/or segmentation labels in AIVI format.

    This class supports loading data for object detection, instance segmentation, pose estimation, and oriented bounding
    box (OBB) tasks using the AIVI format.
    ...
    Methods:
        ...
        get_labels: Return list of label dictionaries for AIVI training.
    ...
    Examples:
        >>> dataset = AIVIDataset(img_path="path/to/images", data={"names": {0: "person"}}, task="detect")
        >>> dataset.get_labels()
    """

    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        """Initialize the AIVIDataset.
    ...
    def get_labels(self) -> list[dict]:
        """Return list of label dictionaries for AIVI training.
...
class AIVIMultiModalDataset(AIVIDataset):
    """Dataset class for loading object detection and/or segmentation labels in AIVI format with multi-modal support.

    This class extends AIVIDataset to add text information for multi-modal model training, enabling models to process
    ...
    Examples:
        >>> dataset = AIVIMultiModalDataset(img_path="path/to/images", data={"names": {0: "person"}}, task="detect")
        >>> batch = next(iter(dataset))
        >>> print(batch.keys())  # Should include 'texts'
    """

    def __init__(self, *args, data: dict | None = None, task: str = "detect", **kwargs):
        """Initialize a AIVIMultiModalDataset.
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        if self.augment:
            ...
            # NOTE: this implementation is different from official aivie,
...
class GroundingDataset(AIVIDataset):
...
class AIVIConcatDataset(ConcatDataset):
    """Dataset as a concatenation of multiple datasets.

    This class is useful to assemble different existing datasets for AIVI training, ensuring they use the same collation
    function.

    Methods:
        collate_fn: Static method that collates data samples into batches using AIVIDataset's collation function.

    Examples:
        >>> dataset1 = AIVIDataset(...)
        >>> dataset2 = AIVIDataset(...)
        >>> combined_dataset = AIVIConcatDataset([dataset1, dataset2])
    """
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        Returns:
            (dict): Collated batch with stacked tensors.
        """
        return AIVIDataset.collate_fn(batch)
...
class SemanticDataset(AIVIDataset):
...
class PolygonSemanticDataset(SemanticDataset, AIVIDataset):
    """Semantic segmentation dataset that rasterizes AIVI polygon labels into masks on the fly.
    ...
    def get_labels(self):
        """Parse AIVI polygon .txt labels."""
        return AIVIDataset.get_labels(self)

    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict[str, Any]:
        """Cache polygon labels via AIVIDataset to keep the 5-tuple `results` format expected by get_labels."""
        return AIVIDataset.cache_labels(self, path)
...
class ClassificationDataset:
    ...
    def __init__(self, root: str, args, augment: bool = False, prefix: str = ""):
        """Initialize AIVI classification dataset with root directory, arguments, augmentations, and cache settings.
        ...
        """
        import torchvision  # scope for faster 'import aivision'
        ...
        if self.cache_ram:
            LOGGER.warning(
                ...
                "https://github.com/aivision/aivision/issues/9824, setting `cache_ram=False`."
            )
