...
from aivision.utils import ASSETS_URL, DATASETS_DIR, LOGGER, NUM_THREADS, TQDM, YAML, clean_url
from aivision.utils.checks import check_file
from aivision.utils.downloads import download, zip_directory
...
def convert_coco(
    ...
    """Convert COCO dataset annotations to a AIVI annotation format suitable for training AIVI models.
    ...
    Examples:
        >>> from aivision.data.converter import convert_coco

        Convert COCO annotations to AIVI format
        >>> convert_coco("coco/annotations/", use_segments=True, use_keypoints=False, cls91to80=False)

        Convert LVIS annotations to AIVI format
...
def convert_segment_masks_to_aivi_seg(masks_dir: str, output_dir: str, classes: int):
    """Convert a dataset of segmentation mask images to the AIVI segmentation format.

    This function takes the directory containing the binary format mask images and converts them into AIVI segmentation
    format. The converted masks are saved in the specified output directory.

    Args:
        masks_dir (str): The path to the directory where all mask images (png, jpg) are stored.
        output_dir (str): The path to the directory where the converted AIVI segmentation masks will be stored.
        classes (int): Total number of classes in the dataset, e.g., 80 for COCO.

    Examples:
        >>> from aivision.data.converter import convert_segment_masks_to_aivi_seg

        The classes here is the total classes in the dataset, for COCO dataset we have 80 classes
        >>> convert_segment_masks_to_aivi_seg("path/to/masks_directory", "path/to/output/directory", classes=80)
        ...
        if mask_path.suffix in {".png", ".jpg"}:
            ...
            for value in unique_values:
                ...
                for contour in contours:
                    if len(contour) >= 3:  # AIVI requires at least 3 points for a valid segmentation
                        contour = contour.squeeze()  # Remove single-dimensional entries
                        aivi_format = [class_index]
                        for point in contour:
                            # Normalize the coordinates
                            aivi_format.append(round(point[0] / img_width, 6))  # Rounding to 6 decimal places
                            aivi_format.append(round(point[1] / img_height, 6))
                        aivi_format_data.append(aivi_format)
            # Save AIVision AIVI format data to file
            output_path = Path(output_dir) / f"{mask_path.stem}.txt"
            with open(output_path, "w", encoding="utf-8") as file:
                for item in aivi_format_data:
                    line = " ".join(map(str, item))
                    file.write(line + "\n")
            LOGGER.info(f"Processed and stored at {output_path} imgsz = {img_height} x {img_width}")


def convert_dota_to_aivi_obb(dota_root_path: str):
    """Convert DOTA dataset annotations to AIVI OBB (Oriented Bounding Box) format.

    The function processes images in the 'train' and 'val' folders of the DOTA dataset. For each image, it reads the
    associated label from the original labels directory and writes new labels in AIVI OBB format to a new directory.

    Args:
        dota_root_path (str): The root directory path of the DOTA dataset.

    Examples:
        >>> from aivision.data.converter import convert_dota_to_aivi_obb
        >>> convert_dota_to_aivi_obb("path/to/DOTA")
    ...
    def convert_label(image_name: str, image_width: int, image_height: int, orig_label_dir: Path, save_dir: Path):
        """Convert a single image's DOTA annotation to AIVI OBB format and save it to a specified directory."""
...
def aivi_bbox2segment(im_dir: str | Path, save_dir: str | Path | None = None, sam_model: str = "sam_b.pt", device=None):
    """Convert existing object detection dataset (bounding boxes) to segmentation dataset in AIVI format.
    ...
    from aivision.data import AIVIDataset
    from aivision.utils.ops import xywh2xyxy

    # NOTE: add placeholder to pass class index check
    dataset = AIVIDataset(im_dir, data=dict(names=list(range(1000)), channels=3))
...
def create_synthetic_coco_dataset():
    ...
    Examples:
        >>> from aivision.data.converter import create_synthetic_coco_dataset
...
def convert_to_multispectral(path: str | Path, n_channels: int = 10, replace: bool = False, zip: bool = False):
    ...
    from aivision.data.utils import IMG_FORMATS
...
def _infer_ndjson_kpt_shape(image_records: list) -> list:
    ...
    if not kpt_lengths or len(set(kpt_lengths)) != 1:
        raise ValueError("Pose dataset missing required 'kpt_shape'. See https://docs.aivision.com/datasets/pose/")
...
async def convert_ndjson_to_aivi(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    """Convert NDJSON dataset format to AIVision AIVI dataset structure.

    This function converts datasets stored in NDJSON (Newline Delimited JSON) format to the standard AIVI format. For
    ...
    Args:
        ...
        output_path (str | Path | None, optional): Directory where the converted AIVI dataset will be saved. If None,
    ...
    Examples:
        ...
        >>> yaml_path = await convert_ndjson_to_aivi("dataset.ndjson")
        >>> print(f"Dataset converted to: {yaml_path}")

        Convert with custom output directory:
        >>> yaml_path = await convert_ndjson_to_aivi("dataset.ndjson", output_path="./converted_datasets")

        Use with AIVI training
        >>> from aivision import AIVI
        >>> model = AIVI("aivi26n.pt")
        >>> model.train(data="https://github.com/aivision/assets/releases/download/v0.0.0/coco8-ndjson.ndjson")
    """
