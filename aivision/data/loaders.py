...
from aivision.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from aivision.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from aivision.utils.checks import check_requirements
from aivision.utils.patches import imread
...
class LoadScreenshots:
    """AIVision screenshot dataloader for capturing and processing screen images.

    This class manages the loading of screenshot images for processing with AIVI. It is suitable for use with `aivi
...
class LoadImagesAndVideos:
    """A class for loading and processing images and videos for AIVI object detection.
    ...
    def __init__(self, path: str | Path | list, batch: int = 1, vid_stride: int = 1, channels: int = 3):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() https://github.com/aivision/aivision/issues/2912
            if "*" in a:
...
def autocast_list(source: list[Any]) -> list[Image.Image | np.ndarray]:
    """Convert a list of sources into a list of numpy arrays or PIL images for AIVision prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported AIVision prediction source type. \n"
                f"See https://docs.aivision.com/modes/predict for supported source types."
            )
