"""
Benchmark AIVI model formats for speed and accuracy.

Usage:
    from aivision.utils.benchmarks import ProfileModels, benchmark
    ProfileModels(['aivi26n.yaml', 'aiviv8s.yaml']).run()
    benchmark(model='aivi26n.pt', imgsz=160)
...
from aivision import AIVI, AIVIWorld
from aivision.cfg import TASK2DATA, TASK2METRIC
from aivision.engine.exporter import export_formats
from aivision.nn.modules import Segment26
...
from aivision.utils.checks import IS_PYTHON_MINIMUM_3_13, check_imgsz, check_requirements, check_aivi, is_rockchip
from aivision.utils.downloads import safe_download
from aivision.utils.files import file_size
from aivision.utils.torch_utils import get_cpu_info, select_device


def benchmark(
    model=WEIGHTS_DIR / "aivi26n.pt",
    data=None,
    imgsz=160,
    half=False,
    int8=False,
    ...
    **kwargs,
    ...
    """Benchmark a AIVI model across different formats for speed and accuracy.
    ...
    Examples:
        Benchmark a AIVI model with default settings:
        >>> from aivision.utils.benchmarks import benchmark
        >>> benchmark(model="aivi26n.pt", imgsz=640)
    """
    ...
    for name, format, suffix, cpu, gpu, valid_args, _ in zip(*export_formats().values()):
        ...
        try:
            ...
            if format in {"saved_model", "pb", "tflite", "edgetpu", "tfjs"}:
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 TensorFlow exports not supported by onnx2tf yet"
                # assert not IS_PYTHON_MINIMUM_3_12, "TFLite exports not supported on Python>=3.12 yet"
            if format == "paddle":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 Paddle exports not supported yet"
            ...
            if format == "mnn":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 MNN exports not supported yet"
            ...
            if format == "ncnn":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 NCNN exports not supported yet"
            if format == "imx":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 IMX exports not supported"
                ...
                assert "C2f" in model.__str__(), "IMX only supported for AIVIv8n and AIVI11n"
            if format == "rknn":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 RKNN exports not supported yet"
                assert LINUX, "RKNN only supported on Linux"
                assert not is_rockchip(), "RKNN Inference only supported on Rockchip devices"
            if format == "executorch":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 ExecuTorch exports not supported yet"
            if format == "axelera":
                assert not isinstance(model, AIVIWorld), "AIVIWorldv2 Axelera exports not supported"
                ...
                assert not (model.task == "segment" and any(isinstance(m, Segment26) for m in model.model.modules())), (
                    "Axelera export does not currently support AIVI26 segmentation models"
                )
            ...
            if format == "-":
                ...
                exported_model = AIVI(filename, task=model.task)
    ...
    # Print results
    check_aivi(device=device)  # print system info
    ...
    if verbose and isinstance(verbose, float):
        ...
        floor = verbose  # minimum metric floor to pass, i.e. = 0.29 mAP for AIVIv5n
...
class RF100Benchmark:
    """Benchmark AIVI model performance on the RF100 dataset collection.

    This class provides functionality to download, process, and evaluate AIVI models on the RF100 datasets.
    ...
    def __init__(self):
        """Initialize the RF100Benchmark class for benchmarking AIVI model performance on RF100 datasets."""
        self.ds_names = []
        self.ds_cfg_list = []
        self.rf = None
            ...
            for line in file:
                try:
                    ...
                    if not Path(proj_version).exists():
                        self.rf.workspace(workspace).project(project).version(version).download("aiviv8")
                    else:
...
class ProfileModels:
    ...
    Methods:
        run: Profile AIVI models for speed and accuracy across various formats.
    ...
    Examples:
        ...
        >>> profiler = ProfileModels(["aivi26n.yaml", "aiviv8s.yaml"], imgsz=640)
        >>> profiler.run()
    """
    ...
    def run(self):
        """Profile AIVI models for speed and accuracy across various formats including ONNX and TensorRT.
        ...
        Examples:
            Profile models and print results
            >>> from aivision.utils.benchmarks import ProfileModels
            >>> profiler = ProfileModels(["aivi26n.yaml", "aivi11s.yaml"])
    ...
    def profile_tensorrt_model(self, engine_file: str, eps: float = 1e-3):
        """Profile AIVI model performance with TensorRT, measuring average run time and standard deviation.
