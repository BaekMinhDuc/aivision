...
from aivision.utils import LOGGER, RANK, SETTINGS, TESTS_RUNNING, ops
from aivision.utils.metrics import ClassifyMetrics, DetMetrics, OBBMetrics, PoseMetrics, SegmentMetrics
...
try:
    ...
    # Names of plots created by AIVision that are logged to Comet
...
def _get_comet_model_name() -> str:
    """Return the Comet model name from environment variable or default to 'AIVision'."""
    return os.getenv("COMET_MODEL_NAME", "AIVision")
...
def _resume_or_create_experiment(args: SimpleNamespace) -> None:
    ...
    try:
        ...
        experiment.log_other("Created from", "aivision")
...
def _fetch_trainer_metadata(trainer) -> dict:
    """Return metadata for AIVI training including epoch and asset saving status.

    Args:
        trainer (aivision.engine.trainer.BaseTrainer): The AIVI trainer object containing training state and config.
...
def _scale_bounding_box_to_original_image_shape(
    ...
    AIVI resizes images during training and the label values are normalized based on this resized shape. This function
...
def _format_prediction_annotations(image_path, metadata, class_label_map=None, class_map=None) -> dict | None:
    """Format AIVI predictions for object detection visualization.
...
def _log_plots(experiment, trainer) -> None:
    ...
    Args:
        ...
        trainer (aivision.engine.trainer.BaseTrainer): The trainer object containing validation metrics and save
            directory information.

    Examples:
        >>> from aivision.utils.callbacks.comet import _log_plots
...
def on_pretrain_routine_start(trainer) -> None:
    """Create or resume a CometML experiment at the start of a AIVI pre-training routine."""
...
def on_fit_epoch_end(trainer) -> None:
    ...
    Args:
        trainer (BaseTrainer): The AIVI trainer object containing training state, metrics, and configuration.
    ...
    if curr_epoch == 1:
        from aivision.utils.torch_utils import model_info_for_loggers
