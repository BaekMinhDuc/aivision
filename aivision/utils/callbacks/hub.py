...
from aivision.hub import HUB_WEB_ROOT, PREFIX, HUBTrainingSession
from aivision.utils import LOGGER, RANK, SETTINGS
from aivision.utils.events import events


def on_pretrain_routine_start(trainer):
    """Create a remote AIVision HUB session to log local model training."""
...
def on_fit_epoch_end(trainer):
    """Upload training progress metrics to AIVision HUB at the end of each epoch."""
    if session := getattr(trainer, "hub_session", None):
...
def on_model_save(trainer):
    """Upload model checkpoints to AIVision HUB with rate limiting."""
    if session := getattr(trainer, "hub_session", None):
...
def on_train_end(trainer):
    """Upload final model and metrics to AIVision HUB at the end of training."""
    if session := getattr(trainer, "hub_session", None):
