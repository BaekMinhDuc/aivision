...
from aivision.utils import ENVIRONMENT, GIT, LOGGER, PYTHON_VERSION, RANK, SETTINGS, TESTS_RUNNING, Retry, colorstr

PREFIX = colorstr("Platform: ")

# Configurable platform URL for debugging (e.g. AIVISION_PLATFORM_URL=http://localhost:3000)
PLATFORM_URL = os.getenv("AIVISION_PLATFORM_URL", "https://platform.aivision.com").rstrip("/")
...
try:
    ...
    assert SETTINGS.get("platform", False) is True or os.getenv("AIVISION_API_KEY") or SETTINGS.get("api_key")
    _api_key = os.getenv("AIVISION_API_KEY") or SETTINGS.get("api_key")
    assert _api_key  # verify API key is present

    import requests

    from aivision.utils.logger import ConsoleLogger, SystemLogger
...
def resolve_platform_uri(uri, hard=True):
    """Resolve ul:// URIs to signed URLs by authenticating with AIVision Platform.
    ...
    api_key = os.getenv("AIVISION_API_KEY") or SETTINGS.get("api_key")
    if not api_key:
        raise ValueError(f"AIVISION_API_KEY required for '{uri}'. Get key at {PLATFORM_URL}/settings")
    ...
    if r.status_code == 401:
        raise ValueError(f"Invalid AIVISION_API_KEY for '{uri}'")
...
def _upload_model(model_path, project, name, progress=False, retry=1, model_id=None):
    ...
    from aivision.utils.uploads import safe_upload
...
def _get_environment_info():
    """Collect comprehensive environment info using existing aivision utilities."""
    import shutil
