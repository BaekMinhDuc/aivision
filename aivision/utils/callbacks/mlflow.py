"""
MLflow Logging for AIVision AIVI.

This module enables MLflow logging for AIVision AIVI. It logs metrics, parameters, and model artifacts.
...
from aivision.utils import LOGGER, RUNS_DIR, SETTINGS, TESTS_RUNNING, colorstr
...
def on_pretrain_routine_end(trainer):
    ...
    Args:
        trainer (aivision.engine.trainer.BaseTrainer): The training object with arguments and parameters to log.
    ...
    experiment_name = os.environ.get("MLFLOW_EXPERIMENT_NAME") or trainer.args.project or "/Shared/AIVision"
    ...
    try:
        ...
        LOGGER.info(f"{PREFIX}disable with 'aivi settings mlflow=False'")
...
def on_train_end(trainer):
    ...
    if getattr(trainer, "_mlflow_active", False):
        try:
            ...
            LOGGER.info(
                f"{PREFIX}results logged to {mlflow.get_tracking_uri()}\n{PREFIX}disable with 'aivi settings mlflow=False'"
            )
        except Exception as e:
