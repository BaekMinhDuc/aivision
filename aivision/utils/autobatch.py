"""Functions for estimating the best AIVI batch size to use a fraction of the available CUDA memory in PyTorch."""
...
from aivision.utils import DEFAULT_CFG, LOGGER, colorstr
from aivision.utils.torch_utils import autocast, profile_ops
...
def check_train_batch_size(
    ...
    """Compute optimal AIVI training batch size using the autobatch() function.

    Args:
        model (torch.nn.Module): AIVI model to check batch size for.
...
def autobatch(
    ...
    """Automatically estimate the best AIVI batch size to use a fraction of the available CUDA memory.

    Args:
        model (torch.nn.Module): AIVI model to compute batch size for.
        imgsz (int, optional): The image size used as input for the AIVI model.
