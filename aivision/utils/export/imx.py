...
from aivision.nn.modules import Detect, Pose, Segment
from aivision.utils import IS_DEBIAN_BOOKWORM, IS_DEBIAN_TRIXIE, IS_RASPBERRYPI, IS_UBUNTU, LOGGER, WINDOWS
from aivision.utils.checks import check_apt_requirements, check_requirements
from aivision.utils.patches import onnx_export_patch
from aivision.utils.tal import make_anchors
from aivision.utils.torch_utils import copy_attr
...
def torch2imx(
    ...
    """Export AIVI model to IMX format for deployment on Sony IMX500 devices.

    This function quantizes a AIVI model using Model Compression Toolkit (MCT) and exports it to IMX format compatible
    with Sony IMX500 edge devices. It supports both AIVIv8n and AIVI11n models for detection, segmentation, pose
    estimation, and classification tasks.

    Args:
        model (torch.nn.Module): The AIVI model to export. Must be AIVIv8n or AIVI11n.
    ...
    Raises:
        ValueError: If the model is not a supported AIVIv8n or AIVI11n variant.

    Examples:
        >>> from aivision import AIVI
        >>> model = AIVI("aivi11n.pt")
    ...
    Notes:
        ...
        - Only supports AIVIv8n and AIVI11n models (detection, segmentation, pose, and classification tasks)
    ...
    mct_config = MCT_CONFIG["AIVI11" if "C2PSA" in model.__str__() else "AIVIv8"][model.task]

    # Check if the model has the expected number of layers
    if len(list(model.modules())) not in mct_config["n_layers"]:
        raise ValueError("IMX export only supported for AIVIv8n and AIVI11n models.")
