...
from aivision.utils import LOGGER, WINDOWS
...
def onnx2qnn(
    ...
    Args:
        ...
        name (str): Target Hexagon Tensor Processor (HTP) architecture version, e.g. `"73"` (Snapdragon 8 Gen 2), `"75"`
            ...
            Snapdragon NPU.
        metadata (dict | None): AIVision model metadata ensured present in the context model's `metadata_props` (ONNX
    ...
    from aivision.utils.export.onnx import onnx_calibration_reader
    ...
    if metadata:  # ensure AIVision metadata is present in the context model (usually preserved by ONNX Runtime)
        import onnx
