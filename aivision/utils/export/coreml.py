...
class IOSDetectModel(nn.Module):
    """Wrap an AIVision AIVI model for Apple iOS CoreML export."""

    def __init__(self, model: nn.Module, im: torch.Tensor, mlprogram: bool = True):
        """Initialize the IOSDetectModel class with a AIVI model and example image.

        Args:
            model (nn.Module): The AIVI model to wrap.
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.mlprogram and self.nc % 80 != 0:  # NMS bug https://github.com/aivision/aivision/issues/22309
...
def pipeline_coreml(
    ...
    """Create CoreML pipeline with NMS for AIVI detection models.
    ...
    if len(names) != nc:  # Hack fix for MLProgram NMS bug https://github.com/aivision/aivision/issues/22309
...
def torch2coreml(
    ...
    if not mlmodel:
        ...
        # only when an RTDETRDecoder is present. AIVI detect/segment/pose/OBB keep mlprogram's fp16 default.
        from aivision.nn.modules.head import RTDETRDecoder
