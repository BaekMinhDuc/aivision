...
from aivision.utils.git import GitRepo
from aivision.utils.patches import imread, imshow, imwrite, torch_save  # for patches
from aivision.utils.tqdm import TQDM  # noqa
...
ROOT = FILE.parents[1]  # AIVI
ASSETS = ROOT / "assets"  # default images
ASSETS_URL = "https://github.com/aivision/assets/releases/download/v0.0.0"  # assets GitHub URL
DEFAULT_CFG_PATH = ROOT / "cfg/default.yaml"
NUM_THREADS = min(8, max(1, os.cpu_count() - 1))  # number of AIVI multiprocessing threads
AUTOINSTALL = str(os.getenv("AIVI_AUTOINSTALL", True)).lower() == "true"  # global auto-install mode
VERBOSE = str(os.getenv("AIVI_VERBOSE", True)).lower() == "true"  # global verbose mode
SAFE_LOAD = str(os.getenv("AIVISION_SAFE_LOAD", False)).lower() == "true"  # opt-in weights_only model loading
LOGGING_NAME = "aivision"
...
HELP_MSG = """
    Examples for running AIVision:

    1. Install the aivision package:

        pip install aivision
    ...
    2. Use the Python SDK:
        ...
        # Load a model
        model = AIVI("aivi26n.yaml")  # build a new model from scratch
        model = AIVI("aivi26n.pt")  # load a pretrained model (recommended for training)

        # Use the model
        results = model.train(data="coco8.yaml", epochs=3)  # train the model
        results = model.val()  # evaluate model performance on the validation set
        results = model("https://aivision.com/images/bus.jpg")  # predict on an image
        success = model.export(format="onnx")  # export the model to ONNX format

    3. Use the command line interface (CLI):

        AIVision 'aivi' CLI commands use the following syntax:
        ...
        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            aivi detect train data=coco8.yaml model=aivi26n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            aivi segment predict model=aivi26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            aivi detect val model=aivi26n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a AIVI26n classification model to ONNX format at image size 224 by 128 (no TASK required)
...
class DataExportMixin:
    ...
    def to_df(self, normalize=False, decimals=5):
        ...
        """
        import polars as pl  # scope for faster 'import aivision'
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date aivision
            'default.yaml' file.\nPlease update your code with 'pip install -U aivision' and if necessary replace
            {DEFAULT_CFG_PATH} with the latest version from
            https://github.com/aivision/aivision/blob/main/aivision/cfg/default.yaml
            """
        )
...
def plt_settings(rcparams=None, backend="Agg"):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            import matplotlib.pyplot as plt  # scope for faster 'import aivision'
            ...
            if "font.sans-serif" not in rcparams and not wrapper._fonts_registered:
                ...
                # Register any fonts in AIVision config dir (e.g. Arial.Unicode.ttf) with matplotlib
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the AIVision library, setting the appropriate logging level and formatter
    ...
    Examples:
        >>> set_logging(name="aivision", verbose=True)
...
class ThreadingLocked:
    ...
    Examples:
        >>> from aivision.utils import ThreadingLocked
        >>> @ThreadingLocked()
        ... def my_function():
        ...    # Your code here
    """
...
def is_online() -> bool:
    ...
    """
    if str(os.getenv("AIVI_OFFLINE", "")).lower() == "true":
        return False
...
def get_user_config_dir(sub_dir="AIVision"):
    """Return a writable config dir, preferring AIVI_CONFIG_DIR and being OS-aware.
    ...
    """
    if env_dir := os.getenv("AIVI_CONFIG_DIR"):
    ...
    for alt in [Path("/tmp") / sub_dir, Path.cwd() / sub_dir]:
        ...
        if is_dir_writeable(alt.parent):
            ...
            LOGGER.warning(
                f"user config directory '{p}' is not writable, using '{alt}'. Set AIVI_CONFIG_DIR to override."
            )
            return alt
...
IS_UBUNTU = is_ubuntu()
GIT = GitRepo()
USER_CONFIG_DIR = get_user_config_dir()  # AIVision settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """AIVision TryExcept class for handling exceptions gracefully.
...
def set_sentry():
    ...
    Only used if sentry_sdk package is installed and sync=True in settings. Run 'aivi settings' to see and update
    settings.

    Conditions required to send errors (ALL conditions must be met or no errors will be reported):
        - sentry_sdk package is installed
        - sync=True in AIVI settings
        - pytest is not running
        - running in a pip package installation
        - running in a non-git directory
        - running with rank -1 or 0
        - online environment
        - CLI used to run package (checked with 'aivi' as the name of the main CLI command)
    """
    if (
        not SETTINGS["sync"]
        or RANK not in {-1, 0}
        or Path(ARGV[0]).name != "aivi"
        or TESTS_RUNNING
        or not ONLINE
...
class SettingsManager(JSONDict):
    """SettingsManager class for managing and persisting AIVision settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.6"):
        ...
        from aivision.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView AIVision Settings with 'aivi settings' or at '{self.file}'"
            "\nUpdate Settings with 'aivi settings key=value', i.e. 'aivi settings runs_dir=path/to/dir'. "
            "For help see https://docs.aivision.com/quickstart/#aivision-settings."
        )
        ...
        with torch_distributed_zero_first(LOCAL_RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new AIVision Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "AIVision settings reset to default values. This may be due to a possible problem "
                f"with your settings or a recent aivision package update. {self.help_msg}"
            )
            self.reset()

        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"AIVision setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No AIVision setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"AIVision setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="aivision.aivision-snippets") -> str:
    """Display a message to install AIVision-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "https://docs.aivision.com/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view AIVision VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("AIVision: ")
