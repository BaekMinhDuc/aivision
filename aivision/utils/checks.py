...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Examples:
        >>> from aivision.utils.checks import parse_requirements
        >>> parse_requirements(package="aivision")
    """
    if package:
...
def check_imgsz(imgsz, stride=32, min_dim=1, max_dim=2, floor=0):
    ...
    if len(imgsz) > max_dim:
        msg = (
            ...
            "or an integer, i.e. 'aivi export imgsz=640,480' or 'aivi export imgsz=640'"
        )
        if max_dim != 1:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='aivision'
        try:
...
def check_latest_pypi_version(package_name="aivision"):
...
def check_pip_update_available():
    """Check if a new version of the aivision package is available on PyPI.
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New https://pypi.org/project/aivision/{latest} available 😃 "
                    f"Update with 'pip install -U aivision'"
                )
                return True
        except Exception:
            pass
...
def check_font(font="Arial.ttf"):
    ...
    """
    from matplotlib import font_manager  # scope for faster 'import aivision'
...
def check_apt_requirements(requirements):
    ...
    if missing_packages:
        LOGGER.info(
            f"{prefix} AIVision requirement{'s' * (len(missing_packages) > 1)} {missing_packages} not found, attempting AutoUpdate..."
        )
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds="", constrain=()):
    """Check if installed dependencies meet AIVision AIVI models requirements and attempt to auto-update if needed.
    ...
    Examples:
        >>> from aivision.utils.checks import check_requirements

        Check a requirements.txt file
        >>> check_requirements("path/to/requirements.txt")

        Check a single package
        >>> check_requirements("aivision>=8.3.200", cmds="--index-url https://download.pytorch.org/whl/cpu")

        Check multiple packages
        >>> check_requirements(["numpy", "aivision"])
    ...
    if os.environ.get("AIVISION_SKIP_REQUIREMENTS_CHECKS", "0") == "1":
        LOGGER.info(f"{prefix} AIVISION_SKIP_REQUIREMENTS_CHECKS=1 detected, skipping requirements check.")
        return True
    ...
    if s:
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} AIVision requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
...
def check_suffix(file="aivi26n.pt", suffix=".pt", msg=""):
...
def check_aiviv5u_filename(file: str, verbose: bool = True) -> str:
    """Replace legacy AIVIv5 filenames with updated AIVIv5u filenames.
    ...
    """
    if "aiviv3" in file or "aiviv5" in file:
        if "u.yaml" in file:
            file = file.replace("u.yaml", ".yaml")  # i.e. aiviv5nu.yaml -> aiviv5n.yaml
        elif ".pt" in file and "u" not in file:
            original_file = file
            file = re.sub(r"(.*aiviv5([nsmlx]))\.pt", "\\1u.pt", file)  # i.e. aiviv5n.pt -> aiviv5nu.pt
            file = re.sub(r"(.*aiviv5([nsmlx])6)\.pt", "\\1u.pt", file)  # i.e. aiviv5n6.pt -> aiviv5n6u.pt
            file = re.sub(r"(.*aiviv3(|-tiny|-spp))\.pt", "\\1u.pt", file)  # i.e. aiviv3-spp.pt -> aiviv3-sppu.pt
            if file != original_file and verbose:
                LOGGER.info(
                    f"PRO TIP 💡 Replace 'model={original_file}' with new 'model={file}'.\nAIVIv5 'u' models are "
                    f"trained with https://github.com/aivision/aivision and feature improved performance vs "
                    f"standard AIVIv5 models trained with https://github.com/aivision/aiviv5."
                )
    return file


def check_model_file_from_stem(model: str = "aivi11n") -> str | Path:
    ...
    if not path.suffix and path.stem in downloads.GITHUB_ASSETS_STEMS:
        return path.with_suffix(".pt")  # add suffix, i.e. aivi26n -> aivi26n.pt
    return model
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True):
    ...
    file = check_aiviv5u_filename(file)  # aiviv5n -> aiviv5nu
    if (
        not file
        ...
        return file
    elif download and file.lower().startswith("ul://"):  # AIVision Platform URI
        from aivision.utils.callbacks.platform import resolve_platform_uri
        ...
        if uri_path.is_absolute() or ".." in uri_path.parts:
            raise ValueError(f"Unsafe AIVision Platform URI path: {file}")
...
def check_aivi(verbose=True, device=""):
    """Print a human-readable AIVI software and hardware summary.

    Args:
        verbose (bool): Whether to print verbose information.
        device (str | torch.device): Device to use for AIVI.
    """
    import psutil  # scoped as slow import

    from aivision.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from aivision.utils import ENVIRONMENT  # scope to avoid circular import
    from aivision.utils.torch_utils import get_cpu_info, get_gpu_info
    ...
    package_info = {}
    for r in parse_requirements(package=get_distribution_name("aivision")):
        try:
...
def check_amp(model):
    """Check the PyTorch Automatic Mixed Precision (AMP) functionality of a AIVI model.

    If the checks fail, it means there are anomalies with AMP on the system that may cause NaN losses or zero-mAP
    results, so AMP will be disabled during training.

    Args:
        model (torch.nn.Module): A AIVI model instance.

    Returns:
        (bool): Returns True if the AMP functionality works correctly with AIVI model, else False.

    Examples:
        >>> from aivision import AIVI
        >>> from aivision.utils.checks import check_amp
        >>> model = AIVI("aivi26n.pt").model.cuda()
        >>> check_amp(model)
    """
    from aivision.utils.torch_utils import autocast
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load AIVI26n for AMP checks due to possible AIVision package modifications. {warning_msg}"
        )
...
def check_multiple_install():
    """Check if there are multiple AIVision installations."""
    import sys

    try:
        result = subprocess.run([sys.executable, "-m", "pip", "show", "aivision"], capture_output=True, text=True)
        install_msg = (
            f"Install your local copy in editable mode with 'pip install -e {ROOT.parent}' to avoid "
            "issues. See https://docs.aivision.com/quickstart/"
        )
        if result.returncode != 0:
            if "not found" in result.stderr.lower():  # Package not pip-installed but locally imported
                LOGGER.warning(f"AIVision not found via pip but importing from: {ROOT}. {install_msg}")
            return
        aivi_path = (Path(re.findall(r"location:\s+(.+)", result.stdout, flags=re.I)[-1]) / "aivision").resolve()
        if not aivi_path.samefile(ROOT.resolve()):
            LOGGER.warning(
                f"Multiple AIVision installations detected. The `aivi` command uses: {aivi_path}, "
...
def print_args(args: dict | None = None, show_file=True, show_func=False):
    ...
    def strip_auth(v):
        """Clean longer AIVision HUB URLs by stripping potential authentication information."""
...
def is_intel():
    ...
    """
    from aivision.utils.torch_utils import get_cpu_info
