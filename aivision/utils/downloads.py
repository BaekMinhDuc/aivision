...
from aivision.utils import ASSETS_URL, LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define AIVision GitHub assets maintained at https://github.com/aivision/assets
GITHUB_ASSETS_REPO = "aivision/assets"
GITHUB_ASSETS_NAMES = frozenset(
    [f"aiviv8{k}{suffix}.pt" for k in "nsmlx" for suffix in ("", "-cls", "-seg", "-pose", "-obb", "-oiv7")]
    + [f"aivi11{k}{suffix}.pt" for k in "nsmlx" for suffix in ("", "-cls", "-seg", "-pose", "-obb")]
    + [f"aivi12{k}{suffix}.pt" for k in "nsmlx" for suffix in ("",)]  # detect models only currently
    + [f"aivi26{k}{suffix}.pt" for k in "nsmlx" for suffix in ("", "-cls", "-seg", "-sem", "-pose", "-obb")]
    + [f"aiviv5{k}{resolution}u.pt" for k in "nsmlx" for resolution in ("", "6")]
    + [f"aiviv3{k}u.pt" for k in ("", "-spp", "-tiny")]
    + [f"aiviv8{k}-world.pt" for k in "smlx"]
    + [f"aiviv8{k}-worldv2.pt" for k in "smlx"]
    + [f"aivie-v8{k}{suffix}.pt" for k in "sml" for suffix in ("-seg", "-seg-pf")]
    + [f"aivie-11{k}{suffix}.pt" for k in "sml" for suffix in ("-seg", "-seg-pf")]
    + [f"aivie-26{k}{suffix}.pt" for k in "nsmlx" for suffix in ("-seg", "-seg-pf")]
    + [f"aiviv9{k}.pt" for k in "tsmce"]
    + [f"aiviv10{k}.pt" for k in "nsmblx"]
    + [f"aivi_nas_{k}.pt" for k in "sml"]
...
def delete_dsstore(path: str | Path, files_to_delete: tuple[str, ...] = (".DS_Store", "__MACOSX")) -> None:
    ...
    Examples:
        >>> from aivision.utils.downloads import delete_dsstore
...
def unzip_file(
    ...
    Examples:
        >>> from aivision.utils.downloads import unzip_file
...
def get_google_drive_file_info(link: str) -> tuple[str, str | None]:
    ...
    Examples:
        >>> from aivision.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    Examples:
        ...
        >>> link = "https://aivision.com/assets/bus.jpg"
    ...
    if "://" not in url and Path(url).is_file():  # local file path ('://' check required in Windows Python<3.10)
        ...
        if not f.is_file():  # URL and file do not exist
            uri = (url if gdrive else clean_url(url)).replace(ASSETS_URL, "https://aivision.com/assets")  # clean
...
def get_github_assets(
    ...
    Examples:
        >>> tag, assets = get_github_assets(repo="aivision/assets", version="latest")
    """
    import requests  # scoped as slow import
    ...
    if r.status_code != 200:
        ...
        return "", []
    data = r.json()
    return data["tag_name"], [x["name"] for x in data["assets"]]  # tag, assets i.e. ['aivi26n.pt', 'aivi11s.pt', ...]
...
def attempt_download_asset(
    ...
    **kwargs,
    ...
    Examples:
        >>> file_path = attempt_download_asset("aivi26n.pt", repo="aivision/assets", release="latest")
    """
    from aivision.utils import SETTINGS  # scoped for circular import

    # AIVIv3/5u updates
    file = str(file)
    file = checks.check_aiviv5u_filename(file)
    ...
    if file.exists():
        ...
        # Server weight download disabled in AIVision: only local files are resolved.
        raise FileNotFoundError(
            f"'{file}' not found locally. AIVision does not download weights from a server. "
            f"Provide a local model file (.pt) for inference, or a .yaml config to train from scratch."
        )
...
def download(
    ...
    Examples:
        >>> download("https://github.com/aivision/assets/releases/download/v0.0.0/bus.jpg", dir="path/to/dir")
    """
    dir = Path(dir)
