...
from aivision.utils.cpu import CPUInfo
...
if WINDOWS and check_version(TORCH_VERSION, "==2.4.0"):  # reject version 2.4.0 on Windows
    LOGGER.warning(
        ...
        "https://github.com/aivision/aivision/issues/15049"
    )
...
def get_cpu_info():
    ...
    from aivision.utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", newline=False, verbose=True):
    ...
    s = f"AIVision {__version__} 🚀 Python-{PYTHON_VERSION} torch-{TORCH_VERSION} "
...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    model_name = Path(yaml_file).stem.replace("aivi", "AIVI") or "Model"
    LOGGER.info(f"{model_name} summary{fused}: {n_l:,} layers, {n_p:,} parameters, {n_g:,} gradients{fs}")
    return n_l, n_p, n_g, flops


def get_num_params(model):
    """Return the total number of parameters in a AIVI model."""
    return sum(x.numel() for x in model.parameters())


def get_num_gradients(model):
    """Return the total number of parameters with gradients in a AIVI model."""
...
def model_info_for_loggers(trainer):
    ...
    Args:
        trainer (aivision.engine.trainer.BaseTrainer): The trainer object containing model and validation data.

    Returns:
        (dict): Dictionary containing model parameters, GFLOPs, and inference speeds.

    Examples:
        AIVIv8n info for loggers
        >>> results = {
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/aivision/aiviv5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    Examples:
        >>> from pathlib import Path
        >>> from aivision.utils.torch_utils import strip_optimizer
    ...
    try:
        ...
        LOGGER.warning(f"Skipping {f}, not a valid AIVision model: {e}")
        return {}
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    """AIVision speed, memory and FLOPs profiler.
    ...
    Examples:
        >>> from aivision.utils.torch_utils import profile_ops
