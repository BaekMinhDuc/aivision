...
from aivision.data.loaders import get_best_youtube_url
...
def run(
    ...
    """Run action recognition on a video source using AIVI for object detection and a video classifier.

    Args:
        weights (str): Path to the AIVI model weights.
    ...
    aivi_model = AIVI(weights).to(device)
    ...
    while cap.isOpened():
        ...
        # Run AIVI tracking
        results = aivi_model.track(frame, persist=True, classes=[0])  # Track only person class
        ...
        cv2.imshow("AIVIv8 Tracking with S3D Classification", frame)
...
def parse_opt() -> argparse.Namespace:
    ...
    parser.add_argument("--weights", type=str, default="aivi11n.pt", help="aivision detector model path")
