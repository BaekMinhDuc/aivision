...
impl OrtBackend {
                ...
                Ok(metadata) => match metadata.custom("task") {
                    ...
                    Ok(value) => match value {
                        ...
                        Some(task) => match task.as_str() {
                            "classify" => AIVITask::Classify,
                            "detect" => AIVITask::Detect,
                            "pose" => AIVITask::Pose,
                            "segment" => AIVITask::Segment,
    ...
    pub fn nc(&self) -> Option<u32> {
        ...
        match self.names() {
            ...
            None => match self.task() {
                // by task calculation
                AIVITask::Classify => Some(self.output_shapes()[0][1] as u32),
                AIVITask::Detect => {
    ...
    pub fn nm(&self) -> Option<u32> {
        // num_masks
        match self.task() {
            AIVITask::Segment => Some(self.output_shapes()[1][1] as u32),
            _ => None,
        }
    }
    ...
    pub fn na(&self) -> Option<u32> {
        // num_anchors
        match self.task() {
            AIVITask::Segment | AIVITask::Detect | AIVITask::Pose => {
