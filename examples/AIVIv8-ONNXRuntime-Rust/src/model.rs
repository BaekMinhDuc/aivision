...
use crate::{
    ...
    OrtConfig, OrtEP, Point2, AIVIResult, AIVITask, SKELETON,
};

pub struct AIVIv8 {
    // AIVIv8 model for all aivi-tasks
    engine: OrtBackend,
    nc: u32,
    nk: u32,
    nm: u32,
...
impl AIVIv8 {
    ...
    pub fn preprocess(&mut self, xs: &Vec<DynamicImage>) -> Result<Array<f32, IxDyn>> {
        ...
        for (idx, x) in xs.iter().enumerate() {
            let img = match self.task() {
                AIVITask::Classify => x.resize_exact(
                    self.width(),
                    self.height(),
                ...
                _ => {
                    ...
                    x.resize_exact(
                        w_new as u32,
                        h_new as u32,
                        if let AIVITask::Segment = self.task() {
    ...
    pub fn run(&mut self, xs: &Vec<DynamicImage>) -> Result<Vec<AIVIResult>> {
        // pre-process
    ...
    pub fn postprocess(
        ...
        xs0: &[DynamicImage],
    ) -> Result<Vec<AIVIResult>> {
        if let AIVITask::Classify = self.task() {
            let mut ys = Vec::new();
            let preds = &xs[0];
            for batch in preds.axis_iter(Axis(0)) {
                ys.push(AIVIResult::new(
            ...
            for (idx, anchor) in preds.axis_iter(Axis(0)).enumerate() {
                ...
                for pred in anchor.axis_iter(Axis(1)) {
                    ...
                    let kpts = {
                        if let AIVITask::Pose = self.task() {
                ...
                for elem in data.into_iter() {
                    ...
                    if let Some(coefs) = elem.2 {
                        ...
                        let mask_original = mask_cropped.resize_exact(
                            ...
                            match self.task() {
                                AIVITask::Segment => image::imageops::FilterType::CatmullRom,
