...
from sahi.utils.aivision import download_model_weights

from aivision.utils.files import increment_path


class SAHIInference:
    """Runs AIVision AIVI11 and SAHI for object detection on video with options to view, save, and track results.

    This class integrates SAHI (Slicing Aided Hyper Inference) with AIVI11 models to perform efficient object detection
    on large images by slicing them into smaller pieces, running inference on each slice, and then merging the results.

    Attributes:
        detection_model (AutoDetectionModel): The loaded AIVI11 model wrapped with SAHI functionality.

    Methods:
        load_model: Load a AIVI11 model with specified weights for object detection using SAHI.
        inference: Run object detection on a video using AIVI11 and SAHI.
    ...
    Examples:
        ...
        >>> sahi_inference.inference(weights="aivi11n.pt", source="video.mp4", view_img=True)
    """

    def __init__(self):
        """Initialize the SAHIInference class for performing sliced inference using SAHI with AIVI11 models."""
        self.detection_model = None

    def load_model(self, weights: str, device: str) -> None:
        """Load a AIVI11 model with specified weights for object detection using SAHI.
        ...
        if weights and os.path.exists(weights):
            aivi11_model_path = weights
        else:
            aivi11_model_path = f"models/{weights}"
            download_model_weights(aivi11_model_path)  # Download model if not present
        self.detection_model = AutoDetectionModel.from_pretrained(
            model_type="aivision", model_path=aivi11_model_path, device=select_device(device)
        )

    def inference(
        self,
        weights: str = "aivi11n.pt",
        source: str = "test.mp4",
        view_img: bool = False,
        save_img: bool = False,
        exist_ok: bool = False,
        ...
        """Run object detection on a video using AIVI11 and SAHI.
        ...
        while cap.isOpened():
            ...
            if view_img:
                cv2.imshow("AIVision AIVI Inference", frame)
    ...
    def parse_opt() -> argparse.Namespace:
        ...
        parser.add_argument("--weights", type=str, default="aivi11n.pt", help="initial weights path")
