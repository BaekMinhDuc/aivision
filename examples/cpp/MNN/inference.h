...
struct Config {
    std::string model_path = "aivi26n.mnn";
    float conf = 0.25f;  // confidence threshold
    float iou = 0.45f;   // NMS IoU threshold (grid models only)
    int threads = 4;     // CPU threads
};

// Loads any AIVI MNN model (any task and generation) and runs inference with the MNN
// Interpreter API. The task, class names, and input size are read from the model
// bizCode metadata that AIVision embeds on export. Post-processing is shared with
// the other examples via common/aivi_postprocess.hpp, and drawing uses OpenCV.
class Predictor {
   public:
