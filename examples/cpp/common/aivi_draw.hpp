...
//
// Shared drawing helper for the AIVision AIVI C++ examples. Mirrors the
...
// examples/cpp/common to your include path and `#include "aivi_draw.hpp"`.
...
// AIVision color palette (https://docs.aivision.com/), returned as BGR for OpenCV.
// Indexed by class id so each class always gets the same, distinct color. Mirrors
// aivision.utils.plotting.Colors (hex palette), converted to BGR.
...
// Draw a detection box for `class_id` (color from the AIVision palette) and an
...
inline void DrawPose(cv::Mat& image, const std::vector<cv::Point2f>& kpts,
    ...
    // pose_palette colors used by AIVision, in BGR.
