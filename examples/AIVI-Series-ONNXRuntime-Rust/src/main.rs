...
use usls::{
    models::AIVI, Annotator, DataLoader, Device, Options, Viewer, Vision, AIVIScale, AIVITask,
    AIVIVersion, COCO_SKELETONS_16,
};
...
pub struct Args {
    ...
    /// Input source path
    #[arg(long, default_value_t = String::from("../../aivision/assets/bus.jpg"))]
    pub source: String,

    /// AIVI Task
    #[arg(long, value_enum, default_value_t = AIVITask::Detect)]
    pub task: AIVITask,

    /// AIVI Version
    #[arg(long, value_enum, default_value_t = AIVIVersion::V8)]
    pub ver: AIVIVersion,

    /// AIVI Scale
    #[arg(long, value_enum, default_value_t = AIVIScale::N)]
    pub scale: AIVIScale,
...
fn main() -> Result<()> {
    ...
    let options = Options::new()
        .with_model(&path)?
        .with_aivi_version(args.ver)
        .with_aivi_task(args.task)
        .with_device(device)
    ...
    // build model
    let mut model = AIVI::new(options)?;
