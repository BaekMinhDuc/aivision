...
from aivision.utils import ASSETS
...
class AIVIv8TFLite:
    """A AIVIv8 object detection class using TensorFlow Lite for efficient inference.

    This class handles model loading, preprocessing, inference, and visualization of detection results for AIVIv8 models
    converted to TensorFlow Lite format.

    Attributes:
        model (Interpreter): TensorFlow Lite interpreter for the AIVIv8 model.
    ...
    Examples:
        ...
        >>> detector = AIVIv8TFLite("aiviv8n.tflite", conf=0.25, iou=0.45)
        >>> result = detector.detect("image.jpg")
        >>> cv2.imshow("Result", result)
    """

    def __init__(self, model: str, conf: float = 0.25, iou: float = 0.45, metadata: str | None = None):
        """Initialize the AIVIv8TFLite detector.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="aiviv8n_saved_model/aiviv8n_full_integer_quant.tflite",
        help="Path to TFLite model.",
    )
    parser.add_argument("--img", type=str, default=str(ASSETS / "bus.jpg"), help="Path to input image")
    parser.add_argument("--conf", type=float, default=0.25, help="Confidence threshold")
    parser.add_argument("--iou", type=float, default=0.45, help="NMS IoU threshold")
    parser.add_argument("--metadata", type=str, default="aiviv8n_saved_model/metadata.yaml", help="Metadata yaml")
    args = parser.parse_args()

    detector = AIVIv8TFLite(args.model, args.conf, args.iou, args.metadata)
