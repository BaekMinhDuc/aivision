...
from aivision.utils.plotting import Annotator, colors
...
counting_regions = [
    {
        "name": "AIVision AIVI Polygon Region",
    ...
    {
        "name": "AIVision AIVI Rectangle Region",
...
def run(
    ...
    """Run object detection and counting within specified regions using AIVI and ByteTrack.
    ...
    Args:
        weights (str): Path to the AIVI model weights file.
    ...
    Examples:
        ...
        >>> run(weights="aivi11s.pt", source="traffic.mp4", classes=[0, 2, 3], device="0")
    """
    vid_frame_count = 0
    ...
    # Output setup
    save_dir = increment_path(Path("aivision_rc_output") / "exp", exist_ok)
    ...
    while videocapture.isOpened():
        ...
        if view_img:
            if vid_frame_count == 1:
                cv2.namedWindow("AIVision AIVI Region Counter Movable")
                cv2.setMouseCallback("AIVision AIVI Region Counter Movable", mouse_callback)
            cv2.imshow("AIVision AIVI Region Counter Movable", frame)
