...
"""
AIVI11 Instance Segmentation with Axelera Voyager SDK.

Standalone example using the axelera-rt pipeline API.
No aivision dependency at runtime.

Usage:
    python aivi11-seg.py --model aivi11n-seg.axm --source 0
    python aivi11-seg.py --model aivi11n-seg.axm --source video.mp4
"""
...
def build_pipeline(model_path: str, conf: float = 0.25, iou: float = 0.45):
    """Build AIVI11 instance segmentation pipeline.
    ...
    return op.seq(
        ...
        op.totensor(),
        model_op,
        op.decode_segmentation(algo="aivi11", num_classes=80, num_mask_coeffs=32, confidence_threshold=conf),
...
def main():
    """AIVI11 Instance Segmentation example."""
    parser = argparse.ArgumentParser(description="AIVI11 Instance Segmentation -- Axelera Voyager SDK")
    ...
    while True:
        ...
        cv2.imshow("AIVI11 Segmentation", annotated)
