...
"""
AIVision AIVI26 Pose Estimation with optional Multi-Object Tracking using Axelera Voyager SDK.

Standalone example using the axelera-rt pipeline API.
No aivision dependency at runtime.

AIVision AIVI26 is NMS-free: the model outputs (1, 300, 57) already in final format
...
supports aiviv8/aivi11 raw output layout).
...
Usage:
    python aivi26-pose-tracker.py --model aivi26n-pose.axm --source 0
    python aivi26-pose-tracker.py --model aivi26n-pose.axm --source video.mp4
    python aivi26-pose-tracker.py --model aivi26n-pose.axm --source video.mp4 --tracker bytetrack
    python aivi26-pose-tracker.py --model aivi26n-pose.axm --source image.jpg --tracker none
"""
...
# Pose color palette (RGB) from AIVision
...
class ConfidenceFilter(op.Operator):
    ...
    Receives the raw output of op.load() -- for AIVision AIVI26-pose this is shaped (1, 300, 57): each row is [x0,
    y0, x1, y1, score, class_id, 17x(kpt_x, kpt_y, kpt_conf)]. Column 4 (score) is used for thresholding.

    This replaces decode_pose which doesn't support AIVision AIVI26's column layout (class_id at column 5 shifts all
    keypoints by one).
    """
...
def build_pipeline(model_path: str, conf: float = 0.25, tracker_algo: str | None = "tracktrack"):
    """Build AIVision AIVI26 pose estimation pipeline with optional tracking.

    AIVision AIVI26 is NMS-free (end-to-end), so no op.nms() is needed. When tracker_algo is provided, op.tracker()
...
def main():
    """AIVision AIVI26 Pose Estimation + Tracking example."""
    parser = argparse.ArgumentParser(description="AIVision AIVI26 Pose Estimation + Tracking - Axelera Voyager SDK")
    ...
    while True:
        ...
        if args.no_display:
            ...
            title = "AIVision AIVI26 Pose Tracking" if use_tracking else "AIVision AIVI26 Pose"
